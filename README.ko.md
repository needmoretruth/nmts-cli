...
> 없이 돌 때의 규칙이 거기 있습니다. (영어입니다.)
>
> **상태: 이릅니다.** 1.0 전에 인터페이스가 더 바뀔 수 있습니다. 이 문서를 쓴 뒤 늘어난
...
- **비밀번호 재설정이 없습니다.** NMTS 키가 곧 계정입니다. 되찾을 수 없고, 파일을 그대로 둔 채
  바꿀 수도 없습니다. NMTS를 포함해 누구도 그 파일을 열 키를 갖고 있지 않기 때문입니다.
...
npm install -g github:needmoretruth/nmts-cli#v0.36.0    # 버전을 고정할 때
...
둡니다. 더 새 것이 있으면 다음 실행이 stderr에 한 줄을 찍습니다. 그 요청에는 NMTS 키도, API 키도,
...
nmts login     # NMTS 키를 여기에 잠가 두고, API 키를 받습니다
nmts ls        # 파일 목록
...
**NMTS 키는 예전에 「계정 코드」라고 부르던 것이고, 바뀐 것은 이름뿐입니다.** 옵션 이름, 환경
...
| | 하는 일 | 묻는가 |
|---|---|---|
| `NMTS_ACCOUNT_CODE_FILE=/path` | 파일에서 NMTS 키를 읽고, 절대 복사하지 않습니다 | 안 묻습니다 |
| `nmts login` | 암호 문구로 잠가 `~/.nmts/credentials.json`에 둡니다 | 안 묻습니다 |
| `nmts login --plain` | 그대로 씁니다. 파일 모드 600 | 한 번, `unsafe-code-storage` |
| `NMTS_ACCOUNT_CODE`에 NMTS 키를 넣기 | 환경에서 바로 씁니다 | 한 번, `plain-env` |

잠가 둔 NMTS 키는 명령마다 암호 문구가 필요합니다. 터미널에서 받거나 `NMTS_PASSPHRASE`에서 읽습니다.
여는 데 1초 못 되는 시간과 64 MiB 메모리가 들고, 그것이 암호 문구 추측을 비싸게 만듭니다. 다만
암호 문구는 나와 같은 사용자로 도는 것에서는 NMTS 키를 지켜 주지 못합니다. 에이전트가 지켜보는 사람
없이 도는 기계에서는 암호 문구도 손에 닿는 곳에 있어야 하기 때문입니다. 그래서 에이전트에게는
파일 형태를 권합니다 — NMTS 키가 어디에도 복사되지 않고, 권한은 호스트가 정합니다.
...
에이전트에게는 본 계정이 아니라 `nmts create`로 따로 만든 계정을 주십시오.
...
| `nmts login` / `logout` | 이 기계에 NMTS 키와 API 키를 두거나 지웁니다 |
| `nmts whoami` | 저장된 NMTS 키가 어느 계정의 것인지 — 오프라인. `--reveal`은 NMTS 키를 찍습니다 |
| `nmts ls` | 파일 목록 |
...
| `nmts delete-account` | **사람이** 이 계정의 서버 기록을 지웁니다 — 되돌릴 수 없습니다. 이 기계에 NMTS 키가 있어야 하고 문장을 입력해 확인합니다. auto 모드 둘에서는 거절되고, skip-permissions에서만 `--reason`과 함께 됩니다. 저장 네트워크의 바이트와 이 기계의 NMTS 키는 건드리지 않습니다 |
| `nmts accept-terms` | 새 판의 약관을 읽은 뒤 동의합니다: 사람이 판 이름을 직접 입력하거나, 에이전트가 사람에게 물은 뒤 `--accept-terms <판> --accept-privacy <판> --yes`로 답을 옮깁니다 |
| `nmts key new` | NMTS 키만으로 이 계정의 API 키를 만듭니다 — 브라우저 없이. `--scopes read,write,spend`, `--days <n>`. 새 API 키는 이 기기의 자격으로 저장되고, `--print`를 주면 한 번 찍어 줍니다 |
...
`--pay wallet`은 크레딧 대신 **NMTS 키가 만드는 지갑에서** 저장 공간을 삽니다:
...
보낼 수 없습니다. `--publish`가 그것을 영구히 씁니다. NMTS 키에서 나오므로 고를 수도 바꿀 수도
없습니다. NMTS 키가 아니고, 그것만으로는 아무것도 열지 못합니다.
...
키 목록을 보여 주고, 각 키는 묻기 전에 무엇을 여는지, 무엇이 잘못될 수 있는지, 무엇을 덮지 않는지를
...
[CONTRIBUTING.ko.md](CONTRIBUTING.ko.md)가 어떻게 여기 닿는지 말하고, [기여자 동의서](CLA.ko.md)는
