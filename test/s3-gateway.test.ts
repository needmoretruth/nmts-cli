// The local S3 gateway reading a drive, driven over real HTTP by a signed client.
//
...
//
// ⚠ WRITING IS IN `s3-gateway-write.test.ts` and many buckets in `s3-gateway-buckets.test.ts`,
//   because each needs a gateway of its own. The stub drive all three read is `s3-gateway-drive.ts`.
...
import { CONTENT, CREDENTIAL, readOnly } from "./s3-gateway-drive.ts";
...
test("⛔ there is no way to serve this to anybody else", async () => {
  ...
  const sources = ["../src/s3/server.ts", "../src/s3/routes.ts", "../src/commands/s3.ts"].map((rel) =>
