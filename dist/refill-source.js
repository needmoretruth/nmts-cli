...
import { buildIndex, entryAt, fullPathOf, isLive, KIND_FILE, normalisePath } from "./drive-paths.js";
 ...
 */
export function findOriginal(entries, path) {
...
export async function downloadForRefill(input) {
    ...
    if (dekWrapped === undefined)
        throw new NmtsError("unreachable: an original with no key");
    ...
    const remove = () => rmSync(localPath, { force: true });
    try {
        await fetchFile({
            base: input.server,
            apiKey: input.apiKey,
 ...
 */
export async function settleRefill(input, original, newItemId) {
    const at = Date.now();
