...
import { addWallet, listOpeners, removeWallet } from "../openers.js";
import { walletFromKeyFile } from "../openers-key-file.js";
...
export async function openers(verb, args, options = {}) {
    const say = options.write ?? ((line) => process.stdout.write(`${line}\n`));
    const asked = (verb ?? "").trim();
    if (asked === "" || asked === "list")
        return await list(args, say);
    if (asked === "add")
        return await add(args, say, options);
    if (asked === "remove")
        return await remove(args, say);
...
async function list(args, say) {
    ...
    for (const one of listing.openers)
        say(`  ${one.locator}  ${one.kind}  ${one.createdAt}`);
...
async function remove(args, say) {
 ...
 *   right only by luck.
 */
export function walletOpenerFrom(args, options = {}) {
