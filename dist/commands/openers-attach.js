...
import { addWallet, refuseIfWalletOpensAnAccount, } from "../openers.js";
import { BINARY_NAME } from "../product.js";
import { registrationProofOf } from "../registration.js";
import { OPENER_HINTS } from "./openers.js";
...
export async function attachWalletToNewAccount(input) {
    ...
    const attached = await addWallet({ server: input.server, apiKey, accountProof: authSecret }, input.code, input.wallet, OPENER_HINTS);
 ...
 */
export async function refuseIfWalletIsTaken(server, wallet) {
 ...
 */
export function sayWalletAttached(say, attached) {
    say(``);
...
async function revoked(server, proven, keyId) {
    try {
...
function field(answer, name) {
    const value = typeof answer === "object" && answer !== null ? Reflect.get(answer, name) : undefined;
