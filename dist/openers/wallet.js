...
import { fetchSlot, KIND_WALLET, putOpener, removeOpener, } from "./doors.js";
import { withRepeatedSignature, withSignature } from "./sign.js";
/**
 ...
 */
export async function signInWithWallet(server, input, hints = {}) {
    const glue = await loadCrypto();
    return withSignature(input, async (serialized) => {
 ...
 */
export async function addWallet(access, accountCode, input, hints = {}) {
    const glue = await loadCrypto();
    return withRepeatedSignature(input, async (serialized) => {
        const locator = toBase64Url(glue.opener_locator(serialized));
        const taken = await fetchSlot(access.server, locator);
        if (taken !== null)
            refuseTakenName(glue, serialized, taken, accountCode, input.account ?? 1, hints);
 ...
 *    account's.
 */
export async function refuseIfWalletOpensAnAccount(server, input, hints = {}) {
    const glue = await loadCrypto();
    await withSignature(input, async (serialized) => {
        const locator = toBase64Url(glue.opener_locator(serialized));
        if ((await fetchSlot(server, locator)) !== null)
            throw opensAnother(input.account ?? 1, hints);
    }, hints);
}
/**
 ...
 */
function refuseTakenName(glue, serialized, slot, accountCode, account, hints) {
    let opened = null;
    let mine = null;
    try {
        try {
...
function opensAnother(account, hints) {
    return new NmtsError(`WALLET_OPENS_ANOTHER_ACCOUNT: under account number ${account}, this wallet already opens another NMTS account.`, {
        exitCode: 4,
        nextStep: hints.opensAnother ??
 ...
 */
export async function removeWallet(access, locator) {
 ...
 */
export async function walletSlot(server, locator) {
...
function requiredLocator(locator) {
    if (locator.trim() === "") {
 ...
 *   account itself.
 */
function keyBytes(parse, accountCode) {
    try {
