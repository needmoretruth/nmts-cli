...
export async function standingTipAfter(input) {
    ...
    try {
        ...
        if (config.devAddress !== TIP_ADDRESS[input.network] && input.trustServerAddress !== true) {
            input.say(`  Your standing ${percentText(tenths)} % gift was not sent: this server named a different ` +
                `receiving address than the one built into this tool for ${input.network}.`);
            input.say(`  Nothing was signed. On a server you run yourself, --trust-server-tip-address sends to ` +
