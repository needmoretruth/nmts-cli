...
export function adviseFor(env: Environment, hasStoredCode: boolean): Advice[] {
  ...
  if (!env.privateStorage) {
    out.push({
      level: "warn",
      text:
        `This filesystem does not keep the mode a file is written with, so a stored NMTS key ` +
  ...
  if (env.containment !== "none" && env.containment !== "unknown") {
    ...
    out.push({
      level: "warn",
      text:
        `Do not pass the NMTS key as an environment variable in a container: the whole ` +
  ...
  if (!env.interactive && !hasStoredCode) {
    out.push({
      level: "warn",
      text:
        `There is no terminal here, so this tool cannot ask for the NMTS key. It has to ` +
  ...
  if (env.os === "win32") {
    out.push({
      level: "note",
      text:
        `Windows applies no POSIX file mode, so a stored NMTS key inherits the folder's ` +
