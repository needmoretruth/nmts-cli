...
import { unlockCode } from "./code-vault.ts";
import {
  CODE_ENV_VAR,
  PASSPHRASE_ENV_VAR,
  credentialsPath,
...
export async function openAccountCode(options: OpenOptions = {}): Promise<OpenedCode | null> {
  ...
  if (found.source !== "file-locked") return { code: found.code, source: found.source };
 ...
 */
async function readPassphrase(allowPrompt: boolean): Promise<string> {
 ...
 */
export async function requireAccountCode(options: OpenOptions = {}): Promise<OpenedCode> {
  const opened = await openAccountCode(options);
  if (opened === null) throw new NotLoggedInError(BINARY_NAME, CODE_ENV_VAR);
  return opened;
}
