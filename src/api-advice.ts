...
export function adviseFor(code: string): string | null {
  switch (code) {
    ...
    case "ALPHA_NOT_OPEN":
      return (
        "This build asks the server for a channel it does not open. This is not something to " +
        "retry or to fix with a different credential — use a release build."
      );
    case "API_KEY_CAP":
      return (
        "The account holds as many live keys as it is allowed. Nothing here can raise the limit: " +
    ...
    case "OPENER_CAP":
      return (
        "This account already has as many wallets attached as it may. Nothing here can raise that: list " +
        "them with `nmts openers` and remove one that is no longer used, then add this one."
      );
    case "API_KEY_CHANNEL":
      return (
        "This account is enrolled on a preview build, and keys are not issued while it is. Ask " +
    ...
    case "LOCKED_OUT":
      return (
        "Too many failed attempts, so this is shut for a while. Retrying now makes it longer, " +
        "not shorter. The refusal carries the moment it lifts; wait for it."
      );
    case "RATE_LIMITED":
      return (
        "Too many requests too quickly. Wait and send fewer — the refusal carries how long. This " +
        "is not a credential problem, so changing keys will not help."
      );
    case "SURFACE_MISMATCH":
      return (
        "This account acts through a different build than the one calling. The refusal names " +
    ...
    case "TERMS_VERSION_MISMATCH":
      return (
        "The versions sent are not the ones in force; the refusal carries the ones that are. " +
        "This is a stale copy, not a refusal to serve — read the current versions and send those."
      );
    case "TERMS_NOT_IN_FORCE":
      return (
        "There is nothing to accept, so accepting cannot be what is missing. This is a server " +
    ...
    case "TRIAL_LINE_CAPPED":
      return (
        "This internet connection has taken its share of this week's places today — the limit is " +
    ...
    case "CHAIN_REQUEST_REFUSED":
      return (
        "The storage service refused the request itself, so nothing was spent and nothing was " +
    ...
    case "CHAIN_UNCERTAIN":
      return (
        "⛔ Nobody knows whether the storage was registered. Do NOT simply retry: doing so can pay " +
        "twice for the same file. Run `nmts ls` first and see whether the file is there."
      );
    case "CHAIN_SPEND_CAP":
      return (
        "The service has stopped spending on storage for today. This is not about this account " +
        "and no credential or credit changes it. Try tomorrow."
      );
    case "CHAIN_DELETE_FAILED":
      return (
        "The storage could not be released. The file's record is gone from this side either way, " +
        "so nothing here is stuck — the storage runs out on its own when its time is up."
      );
    case "RELEASE_NOT_SPONSORED":
      return (
        "This file's storage was not paid for with credits, so it is not the server's to release. " +
    ...
    case "SPONSORED_STATE":
      return (
        "The upload is not at the step that call belongs to — the steps have an order and one was " +
    ...
    case "ERASE_BLOCKED":
      return (
        "The account cannot be erased while retained records still point at it. This will not " +
