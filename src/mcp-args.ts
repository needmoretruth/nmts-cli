...
function typeMatches(declared: KnownType, value: unknown): boolean {
  switch (declared) {
    case "string":
...
function declaredType(property: unknown): KnownType | null {
 ...
 */
export function unsupported(schema: unknown): string[] {
  if (!isRecord(schema)) return ["the schema is not an object"];
  const found: string[] = [];
  for (const key of Object.keys(schema)) {
    if (!HANDLED_ROOT.includes(key)) found.push(key);
  }
  ...
  if (isRecord(properties)) {
    for (const [name, property] of Object.entries(properties)) {
      ...
      if (declaredType(property) === null) found.push(`${name}.type`);
      for (const key of Object.keys(property)) {
        if (!HANDLED_PROPERTY.includes(key)) found.push(`${name}.${key}`);
      }
    }
  }
  return found;
 ...
 */
export function checkArgs(schema: unknown, args: unknown): string[] {
  if (!isRecord(args)) {
  ...
  for (const name of required) {
    if (!(name in args) || args[name] === undefined) problems.push(`\`${name}\` is required`);
  }
  ...
  for (const [name, value] of Object.entries(args)) {
    ...
    if (property === undefined) {
      if (closed) problems.push(`\`${name}\` is not an argument this tool takes`);
      continue;
    }
    ...
    if (type === "array" && Array.isArray(value) && isRecord(property) && isRecord(property["items"])) {
      ...
      if (itemType !== null) {
        ...
        if (wrong !== -1) {
          problems.push(
            `\`${name}\` must hold ${itemType} values, and item ${wrong + 1} is ${actualType(value[wrong])}`,
          );
        }
      }
    }
