 ...
 */
export const WASHES_ENVIRONMENT: Readonly<Record<HostId, boolean>> = {
  "claude-code": false,
  codex: true,
  opencode: false,
  hermes: true,
 ...
 */
function claudeVersionFrom(aiAgent: string | undefined): string | null {
  if (aiAgent === undefined || !aiAgent.startsWith("claude-code_")) return null;
  const middle = aiAgent.split("_")[1];
  if (middle === undefined || !/^[0-9]+(-[0-9]+)*$/.test(middle)) return null;
 ...
 */
export function hostsInEnvironment(
  env: NodeJS.ProcessEnv = process.env,
): HostSighting[] {
  const out: HostSighting[] = [];
  const seen = (id: HostId, by: string, version: string | null = null) =>
    out.push({ id, relation: "ancestor", by, version });

  if (env["CLAUDECODE"] === "1") seen("claude-code", "CLAUDECODE=1", claudeVersionFrom(env["AI_AGENT"]));
  ...
  if ((env["CODEX_THREAD_ID"] ?? "") !== "") seen("codex", "CODEX_THREAD_ID");
  else if ((env["CODEX_SESSION_ID"] ?? "") !== "") seen("codex", "CODEX_SESSION_ID");
  if (env["OPENCODE"] === "1") seen("opencode", "OPENCODE=1");
  if (env["HERMES_AGENT"] === "true") seen("hermes", "HERMES_AGENT=true");
  else if ((env["HERMES_SESSION_ID"] ?? "") !== "") seen("hermes", "HERMES_SESSION_ID");
  if (env["OPENCLAW_CLI"] === "1") seen("openclaw", "OPENCLAW_CLI=1");
  else if ((env["OPENCLAW_SHELL"] ?? "") !== "") seen("openclaw", "OPENCLAW_SHELL");
 ...
 */
export function hostFromClientInfo(info: ClientInfo | undefined): HostSighting | null {
  const name = typeof info?.name === "string" ? info.name : "";
  if (name === "") return null;
  const version = typeof info?.version === "string" && info.version !== "" ? info.version : null;
  const at = (id: HostId, useVersion: boolean) =>
    ({ id, relation: "parent" as const, by: `clientInfo.name=${name}`, version: useVersion ? version : null });

  if (name === "claude-code") return at("claude-code", true);
  ...
  if (name === "codex-mcp-client") return at("codex", false);
  if (name === "opencode") return at("opencode", true);
  // ⛔ Pinned to "0.0.0" in OpenClaw's main path, so the field is present and meaningless.
  if (name.startsWith("openclaw-")) return at("openclaw", false);
  return null;
}
 ...
 */
export function hermesFromParent(readParentCommand: () => string | null = readLinuxParentCommand): HostSighting | null {
  const command = readParentCommand();
  if (command === null || !command.includes("mcp_stdio_watchdog.py")) return null;
...
export function readLinuxParentCommand(): string | null {
 ...
 */
export function washingHosts(sightings: readonly HostSighting[]): HostId[] {
  const ids = new Set<HostId>();
  for (const s of sightings) if (WASHES_ENVIRONMENT[s.id]) ids.add(s.id);
  return [...ids];
}

/** One sighting in the words the `env` command prints. */
export function describeSighting(s: HostSighting): string {
