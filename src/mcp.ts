...
import { askerFor, type Asker } from "./mcp-ask.ts";
import { renderError } from "./errors.ts";
import { hermesFromParent, hostFromClientInfo, type ClientInfo, type HostSighting } from "./agent-host.ts";
 ...
 */
function isResponse(value: unknown): value is { id: string; result?: unknown; error?: unknown } {
  if (typeof value !== "object" || value === null) return false;
  const v = value as Record<string, unknown>;
 ...
 */
export function clientOf(request: unknown, readParent = hermesFromParent): ClientSighting | null {
  if (!isRequest(request) || request.method !== "initialize") return null;
  const raw = request.params?.["clientInfo"];
  const info: ClientInfo | undefined =
    typeof raw === "object" && raw !== null && !Array.isArray(raw) ? (raw as ClientInfo) : undefined;
 ...
 */
export function capabilitiesOf(request: unknown): unknown {
  if (!isRequest(request) || request.method !== "initialize") return null;
