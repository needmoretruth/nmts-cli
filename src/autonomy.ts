...
export type Autonomy =
  /** The default: the agent asks, and the tool refuses what has not been agreed to. */
  | "off"
  /**
   * The agent decides for itself whether the person asked for this, or whether it is a thing
   * worth doing unasked -- and proceeds when the answer is yes.
   */
  | "auto"
  /** The agent proceeds. There is no judgement step. */
  | "skip-permissions";

export const AUTONOMY_MODES: readonly Autonomy[] = ["off", "auto", "skip-permissions"];
...
function isMode(value: unknown): value is Autonomy {
...
export function currentMode(): Autonomy {
  try {
    ...
    const mode: unknown = Reflect.get(parsed, "mode");
