...
import { AGENTS_DOC, BINARY_NAME, HOME_URL, PRODUCT_NAME, SOURCE_URL, VERSION } from "./product.ts";
...
import { ATTACH_LOG_TEXT, SUPPORT_LONG, supportHelpText } from "./support-copy.ts";
import {
  API_KEY_ENV_VAR,
...
import { optionLines } from "./help-options.ts";
...
export function helpText(version: string): string {
  return [
    ...
    `  losses                Storage the chain lost — a program uploading with a key keeps writing into it`,
    `  ai-account            Accounts made under this one for an AI to work in: list, create, delete.`,
    `                        Each has its own NMTS key and its own empty drive. Needs the NMTS key`,
    ...
    ``,
    `ENVIRONMENT`,
    `  ${CODE_FILE_ENV_VAR.padEnd(24)}Names a FILE holding the NMTS key, read once and never copied. The`,
    `                          recommended way, and how a container is given it (examples/podman.sh).`,
    `  ${CODE_ENV_VAR.padEnd(24)}The NMTS key itself. It wins over that file and over what`,
    `                          \`${BINARY_NAME} login\` stored, and asks once — an environment variable is`,
    `                          readable through \`docker inspect\`, /proc/<pid>/environ and CI logs.`,
 ...
 */
export async function helpFor(topic: string): Promise<{ text: string; found: boolean }> {
  ...
  for (const file of readdirSync(dir).filter((f) => f.endsWith(".md")).sort()) {
    ...
    if (names.includes(wanted)) return { text, found: true };
    covered.push(...names);
  }
  return {
    found: false,
    text:
      `No document for "${topic}". \`${BINARY_NAME} help <command>\` knows: ` +
      `${covered.sort().join(", ")}.\n`,
  };
}
...
export async function printHelp(
  args: { command: string | null; operands: readonly string[] },
