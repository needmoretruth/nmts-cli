...
import type { Certificate, ReserveReply, StatusReply, UploadApi } from "./upload-wire.ts";
...
function reserveReply(value: unknown): ReserveReply {
  ...
  const out: ReserveReply = {
    ledger_id: ledgerId,
    state,
    credits_spent: typeof v["credits_spent"] === "number" ? v["credits_spent"] : 0,
  };
  if (typeof v["blob_object_id"] === "string") out.blob_object_id = v["blob_object_id"];
  if (typeof v["register_tx_digest"] === "string") out.register_tx_digest = v["register_tx_digest"];
  return out;
}

function statusReply(value: unknown): StatusReply {
  const v = object(value, "the reservation");
  const ledgerId = v["ledger_id"];
  const state = v["state"];
  if (typeof ledgerId !== "number" || typeof state !== "string") {
  ...
  const out: StatusReply = { ledger_id: ledgerId, state };
  if (typeof v["blob_object_id"] === "string") out.blob_object_id = v["blob_object_id"];
  if (typeof v["register_tx_digest"] === "string") out.register_tx_digest = v["register_tx_digest"];
  return out;
}

function itemReply(value: unknown): { id: string } {
  ...
  if (typeof id !== "string" || id === "") {
    throw new NmtsError("The file was committed but the server did not say under which id.", {
      nextStep:
        "The bytes are stored and paid for. Running the same command again asks the server again " +
...
export function createUploadApi(base: string, apiKey: string): UploadApi {
  return {
    async reserve(body) {
      return reserveReply(
        await request(base, "/v1/sponsored/reserve", {
          method: "POST",
          token: apiKey,
          body,
    ...
    async uploaded(ledgerId: number, certificate: Certificate) {
