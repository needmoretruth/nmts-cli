...
import { readItemRecord, writeItemRecord } from "./upload-store.ts";
import {
  UploadError,
  type Certificate,
  type CommitInput,
...
export async function pushPart(
  input: UploadInput,
  step: {
    ledgerId: number;
    blobId: string;
  ...
  try {
    ...
    throw new UploadError({
      ...
      paid: true,
      nextStep:
        "The storage is already bought. Running the same command again pushes the same bytes to " +
...
export async function commitItem(
  ...
  //    already named.
  if (previous?.itemId !== undefined) return previous.itemId;
  ...
  try {
    ...
    throw new UploadError({
      ...
      paid: true,
      nextStep:
        "Nothing more will be spent. Running the same command again commits the same stored " +
 ...
 */
function describePart(part: PaidPart, expiryEpoch: number): Record<string, unknown> {
  const common = {
