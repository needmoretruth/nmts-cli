...
import { activeWalletOf, WALLET_INDEX_LIMIT } from "./shared/lib/drive/manifest-settings.ts";
 ...
 */
export function walletIndexOf(raw: string): number {
  const text = raw.trim();
...
export async function payingWalletIndex(input: PayingWalletInput): Promise<number> {
  const said = input.wallet;
  if (said !== undefined && said !== "") return walletIndexOf(said);
  ...
  try {
    ...
    throw new NmtsError(
      `Which wallet should pay is written in this account's file list, and it could not be read: ` +
        `${error instanceof Error ? error.message : String(error)}`,
      {
        exitCode: 1,
        nextStep:
          `Nothing was signed. Try again when the list can be read, or name the wallet for this ` +
...
async function fromSealedList(input: PayingWalletInput): Promise<number> {
