 ...
 */
function looksLikeAnAccountCode(candidate: string): boolean {
  const groups = candidate.split("-");
  if (groups.some((group) => group.length < 3)) return false;
  const symbols = groups.join("");
  if (symbols.length !== ACCOUNT_CODE_SYMBOLS) return false;
...
export const RULES: readonly RedactionRule[] = [
  ...
  },
  {
    ...
    label: "[account-code]",
    test: new RegExp(
      `(?<![0-9A-Za-z])[${DATA_CLASS}]{3,}(?:-[${DATA_CLASS}]{3,})*[*~$=Uu]?(?![0-9A-Za-z])`,
      "gu",
    ),
  ...
  },
  {
    ...
    label: "[account-code]",
    test: new RegExp(
      `(?<![0-9A-Za-z])[${DATA_CLASS}]{4}(?: [${DATA_CLASS}]{4}){6} [${DATA_CLASS}]{4}[${CHECK_CLASS}](?![0-9A-Za-z])`,
      "gu",
    ),
  },
  {
...
function apply(text: string, rule: RedactionRule): string {
  return text.replace(rule.test, (match: string, ...rest: unknown[]): string => {
    if (rule.only !== undefined && !rule.only(match)) return match;
    const groups = rest.filter((v): v is string | undefined => typeof v === "string" || v === undefined);
...
export function redact(text: string): string {
  let out = text;
  for (const rule of environmentRules()) out = apply(out, rule);
 ...
 */
export function omitLiterals(text: string, values: readonly string[]): string {
  let out = text;
  for (const value of values) {
    if (value.length < SHORTEST_OMIT) continue;
