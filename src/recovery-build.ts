...
import {
  buildRecoveryListDoc,
  RecoveryListProblem,
  withTotals,
  type ManifestItemInput,
  type ManifestPartInput,
  type RecoveryDocMetaDraft,
  type RecoveryListDoc,
} from "./recovery-map.ts";
import { STORAGE_QUILT, type SourceItem, type SourcePart } from "./recovery-source.ts";
...
import { buildIndex, KIND_FOLDER, pathOf } from "./shared/lib/drive/manifest-index.ts";
...
export function pathString(names: readonly string[]): string {
  if (names.length === 0) return ROOT_PATH;
 ...
 */
function couldBePadding(len: number, declared: number): boolean {
...
/** What the sealed file list says about one item, in the shape this builder wants. */
interface Described {
  name: string;
  isFolder: boolean;
  size: number;
  dekWrapped: string | undefined;
  contentHashCt: string | undefined;
  /** Folder names from the root down to (and excluding) this item. */
  path: readonly string[];
}

function describeAll(entries: readonly ManifestEntry[]): Map<string, Described> {
...
function openRaw(
  ...
  try {
    ...
    throw new RecoveryListProblem(
      `${what} did not open with this account's key, so it cannot be written down. ` +
...
export function buildRecoveryList(input: BuildRecoveryListInput): BuiltRecoveryList {
  ...
  for (const stored of input.source) {
    ...
    if (about === undefined) {
      throw new RecoveryListProblem(
        `Stored file ${stored.id} is missing from your file list, so it cannot be described. ` +
          `The recovery list was not written.`,
      );
    }
    if (about.isFolder) continue;
    if (about.dekWrapped === undefined) {
      throw new RecoveryListProblem(
        `Stored file ${stored.id} has no stored key, so it could never be opened again. ` +
          `The recovery list was not written.`,
      );
    }
    if (stored.parts.length === 0) {
      throw new RecoveryListProblem(
        `Stored file ${stored.id} has no stored parts, so there would be nothing to fetch. ` +
          `The recovery list was not written.`,
      );
    }
    ...
    if (misnumbered >= 0) {
      throw new RecoveryListProblem(
        `Stored file ${stored.id} has a part numbered ${ordered[misnumbered]?.part_index} where ` +
    ...
    try {
      ...
      throw new RecoveryListProblem(
        `Stored file ${stored.id} has ${ordered.length} stored parts that do not add up to the ` +
          `size in your file list. A part is missing, repeated, or the wrong length. ` +
          `The recovery list was not written.`,
      );
    }

    const parts: ManifestPartInput[] = ordered.map((p, i) => {
      ...
      if (p.streamPlaintextLen > keep && !couldBePadding(keep, p.streamPlaintextLen)) {
        throw new RecoveryListProblem(
          `Stored file ${stored.id} has a part ${i} whose stored stream is not padded by any rule ` +
            `this version writes. The recovery list was not written.`,
        );
      }
      const part: ManifestPartInput = {
        blob_id: p.blob_id,
        plaintext_len: keep,
    ...
    const patch = ordered.find(
      (p) => p.storage_kind === STORAGE_QUILT && p.patch_id !== undefined && p.patch_id.length > 0,
    );
    const item: ManifestItemInput = {
      id: stored.id,
      name: about.name,
    ...
    if (about.contentHashCt !== undefined) {
      ...
      item.contentHash = openRaw(
        input.crypt,
        input.dataKey,
        about.contentHashCt,
        AAD.contentHash,
        `the content hash of ${about.name}`,
      );
    }
  ...
  const sealed = Buffer.from(
    input.crypt.envelope_seal(input.dataKey, utf8.encode(AAD.recoveryMap), utf8.encode(json)),
  ).toString("base64url");
  ...
  try {
    reopened = input.crypt.envelope_open(
      input.dataKey,
      utf8.encode(AAD.recoveryMap),
 ...
 *    really is Walrus.
 */
function nameOfNetwork(part: SourcePart, itemId: string): string {
  const name = networkName(part.network ?? NETWORK_WHEN_UNRECORDED);
  if (name === null) {
    throw new RecoveryListProblem(
      `Stored file ${itemId} is on storage network ${part.network}, which this version does not ` +
