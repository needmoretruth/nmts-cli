 ...
 */
export function fullPathOf(index: ManifestIndex, entry: ManifestEntry): string {
  ...
  if (detached) parts.unshift(DETACHED);
  return parts.join("/");
}
...
export function entryAt(
  ...
  const at = (e: ManifestEntry) => normalisePath(fullPathOf(index, e)) === target;
  const matches = entries.filter(
    (e) =>
      (options.includeTrashed === true || isLive(index, e)) &&
  ...
  if (only === undefined) {
    ...
    throw new NmtsError(
      trashed ? `"${target}" is in the trash.` : `Nothing in this account is at "${target}".`,
      {
        exitCode: 4,
        nextStep: trashed
 ...
 */
export function namesIn(entries: readonly ManifestEntry[], parentId: string | null): Set<string> {
  const taken = new Set<string>();
  for (const e of entries) {
    // A trashed entry still holds its name: restoring it must not land on top of a live one.
    if (e.parentId === parentId) taken.add(normaliseName(e.name));
  }
  return taken;
}
