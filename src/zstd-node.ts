...
import { setZstdCodec, zstdContentSize, type ZstdCodec } from "./shared/lib/drive/zstd.ts";

type Zlib = typeof import("node:zlib");

let loaded: Zlib | null = null;

async function zlib(): Promise<Zlib> {
  if (loaded === null) loaded = await import("node:zlib");
  return loaded;
}
...
const codec: ZstdCodec = {
  ...
  async decompress(bytes: Uint8Array, maxOut: number): Promise<Uint8Array> {
    ...
    if (declared > maxOut) {
      throw new Error(
        `This part of the file list claims ${declared} bytes, over the ${maxOut} allowed.`,
      );
    }
    const z = await zlib();
 ...
 */
export function registerNodeZstd(): void {
...
export function forgetNodeZstd(): void {
  registered = false;
  setZstdCodec(null);
}
