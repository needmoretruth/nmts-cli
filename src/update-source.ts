...
import { SOURCE_URL } from "./product.ts";
...
export function checkingIsOff(env: NodeJS.ProcessEnv = process.env): boolean {
 ...
 */
export function versionFromLocation(location: string): string | null {
  let resolved: string;
  try {
  ...
  if (!resolved.startsWith(TAG_PREFIX)) return null;
 ...
 */
export function packageUrl(version: string): string | null {
 ...
 */
export function installCommand(version: string): readonly string[] | null {
  const url = packageUrl(version);
  return url === null ? null : ["npm", "install", "--global", url];
}

/** The same command as one line, for printing. */
export function installCommandLine(version: string): string | null {
 ...
 */
export function newerVersionLine(latest: string, running: string, binary: string): string {
