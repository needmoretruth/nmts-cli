...
export function readCheck(): UpdateCheck | null {
  ...
  const record: UpdateCheck = { checkedAt };
  ...
  if (typeof latest === "string" && isVersion(latest)) record.latest = latest;
  const failed = parsed["failed"];
  if (typeof failed === "string" && failed.length > 0) record.failed = failed.slice(0, 200);
  return record;
}

/** Write the attempt down. Failing to write is itself ignored: there is nothing to fall back to. */
export function writeCheck(record: UpdateCheck): void {
  try {
...
export function dueForCheck(record: UpdateCheck | null, nowMs: number): boolean {
...
export type Lookup = { version: string } | { failed: string };
 ...
 */
export async function lookupLatest(url: string = LATEST_RELEASE_URL): Promise<Lookup> {
  ...
  if (location === null || location === "") return { failed: "the releases page redirected to nowhere" };
  const version = versionFromLocation(location);
  if (version === null) return { failed: "the newest release is not named in a shape this can read" };
  return { version };
}
 ...
 */
export async function noteUpdate(options: NoteOptions): Promise<void> {
  const env = options.env ?? process.env;
  if (checkingIsOff(env)) return;
  ...
  try {
    ...
    if (!dueForCheck(record, now.getTime())) return;
    const lookup = options.lookup ?? (() => lookupLatest());
    const result = await lookup();
    const written: UpdateCheck = { checkedAt: now.toISOString() };
    if ("version" in result) written.latest = result.version;
    else {
      ...
      if (record?.latest !== undefined) written.latest = record.latest;
    }
    writeCheck(written);
  } catch {
