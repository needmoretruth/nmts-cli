 ...
 */
export function actOf(args: ParsedArgs): ActId | null {
  ...
  switch (c) {
    ...
    default:
      return (ACT_IDS as readonly string[]).includes(c) ? (c as ActId) : null;
  }
}
 ...
 */
export async function gateArgs(args: ParsedArgs): Promise<void> {
  const act = actOf(args);
  if (act === null || args.dryRun) return;
  const { gate } = await import("./gate.ts");
  const passage = await gate(act, args);
  if (!passage.ask) args.yes = true;
}
