 ...
 */
export function readableOnATerminal(bytes: Uint8Array): boolean {
...
export function processStdout(): ByteDestination {
  return {
 ...
 */
export async function handOver(bytes: Uint8Array, to: ByteDestination): Promise<boolean> {
  if (to.isTerminal && !readableOnATerminal(bytes)) {
    throw new NmtsError("These bytes are not text and stdout is a terminal.", {
      exitCode: 4,
      nextStep:
        "Nothing was written. A terminal acts on some of what it is sent, so this refuses rather " +
  ...
  try {
    ...
    throw new NmtsError(
      `The file could not be handed over: ${error instanceof Error ? error.message : String(error)}`,
      {
        exitCode: 1,
        nextStep:
          "Whatever was reading this did not get the whole file. If it was redirected, what is " +
