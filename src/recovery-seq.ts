 ...
 */
function seqPath(accountId: string): string {
...
export function lastOfferedSeq(accountId: string): number {
  try {
    const parsed: unknown = JSON.parse(readFileSync(seqPath(accountId), "utf8"));
 ...
 */
export function rememberOfferedSeq(accountId: string, seq: number): void {
  if (seq <= lastOfferedSeq(accountId)) return;
