...
export async function buyAndPushPart(input: UploadInput): Promise<PaidPart> {
  ...
  if (existing !== null) {
    ...
    if (record.partIndex !== input.part.index || record.partTotal !== input.part.total) {
      throw new UploadError({
        phase: "reserve",
        message:
          `This upload was started as part ${record.partIndex + 1} of ${record.partTotal} and this ` +
  ...
  if (existing?.ledgerId !== undefined) {
    ...
    if (ledgerId === undefined) throw new NmtsError("unreachable: a record with no ledger id");
    ...
    if (!isLive(status.state)) {
      ...
      throw new UploadError({
        ...
        paid: true,
        nextStep:
          "Running the same command again asks for a NEW reservation, which will spend credits. " +
 ...
 */
function paidPart(record: Reservation, ledgerId: number, resumed: boolean): PaidPart {
  return {
...
export function entryOf(record: Reservation): UploadResult["entry"] {
  return {
    name: record.name,
 ...
 */
function stripReservation(record: Reservation): Reservation {
