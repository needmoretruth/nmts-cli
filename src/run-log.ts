...
import { OPTIONS_TAKING_A_VALUE } from "./args.ts";
import { fromUtf8, utf8 } from "./bytes.ts";
import { host } from "./host.ts";
import { redact } from "./redact.ts";
import { VERSION } from "./product.ts";
...
export function runLogIsOff(): boolean {
  return host().env(NO_RUN_LOG_ENV_VAR) === "1";
}

function remember(event: RunEvent): void {
  if (runLogIsOff()) return;
  collected.push(event);
  if (collected.length > MAX_EVENTS) collected = collected.slice(-MAX_EVENTS);
}
...
export function commandOf(argv: readonly string[]): string {
  const takesValue = new Set<string>(OPTIONS_TAKING_A_VALUE);
 ...
 */
export function safeArgs(argv: readonly string[]): string[] {
  const out: string[] = [];
  for (let i = 0; i < argv.length; i += 1) {
    const token = argv[i];
    if (token === undefined) continue;
 ...
 */
export async function recordRun(argv: readonly string[], exit: number, ms: number, now = new Date()): Promise<void> {
  if (runLogIsOff()) {
    forgetRun();
    return;
  }
...
function redactEvent(event: RunEvent): RunEvent {
  if (event.kind === "error") return { kind: "error", message: redact(event.message) };
  const path = redact(event.path);
  if (event.error === undefined) return { kind: "http", method: event.method, path, status: event.status };
...
function isEvent(value: unknown): value is RunEvent {
  ...
  if (kind === "error") return typeof Reflect.get(value, "message") === "string";
 ...
 */
function asRecord(value: unknown): RunRecord | null {
  if (typeof value !== "object" || value === null) return null;
  const at = (name: string): unknown => Reflect.get(value, name);
  const args: unknown = at("args");
  const events: unknown = at("events");
  if (typeof at("t") !== "string" || typeof at("cmd") !== "string") return null;
  if (typeof at("exit") !== "number" || !Array.isArray(args) || !Array.isArray(events)) return null;
  return {
    t: String(at("t")),
...
export async function readRuns(count: number): Promise<RunRecord[]> {
  let text: string;
  try {
