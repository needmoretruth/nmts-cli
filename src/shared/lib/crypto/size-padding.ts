 ...
 */
export function padmeLen(len: number): number {
  if (len < 4) return len;
...
export function pow2Len(len: number): number {
  if (len < 2) return len;
...
export function chunkCount(plaintextLen: number, shape: SealingShape): number {
...
export function sealedLenFor(plaintextLen: number, shape: SealingShape): number {
 ...
 */
export function freeCeiling(len: number, unitBytes: number, shape: SealingShape): number {
  if (unitBytes <= 0) return len;
  ...
  for (;;) {
    ...
    if (candidate >= len && sealedLenFor(candidate, shape) <= paidBytes) return candidate;
    if (chunks <= 1) return len;
    chunks -= 1;
  }
}
 ...
 */
export function paddedPlaintextLen(
  len: number,
  rule: PaddingRule,
  options: { unitBytes: number; shape: SealingShape },
): number {
 ...
 */
export function keepLengths(size: number, streamLens: readonly number[]): number[] {
  ...
  for (let i = 0; i < streamLens.length; i += 1) {
    ...
    if (take > declared) {
      throw new RangeError(
        `part ${i} must contribute ${take} bytes but its stream only holds ${declared}`,
      );
    }
    if (take > remaining) {
      throw new RangeError(
        `part ${i} contributes ${take} bytes but only ${remaining} of the file are left`,
      );
    }
    keep.push(take);
    remaining -= take;
