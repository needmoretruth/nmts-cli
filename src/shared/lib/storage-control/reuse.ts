 ...
 */
export function surelyFits(input: FitInput): boolean {
  if (input.resourceEndEpoch <= input.currentEpoch) return false;
  if (input.heldRawBytes === undefined) return false;
...
export function registerIntoStorage(input: RegisterIntoInput, transaction?: Transaction): Transaction {
 ...
 */
export function splitStorageToFit(input: SplitToFitInput, transaction?: Transaction): Transaction {
  const tx = transaction ?? new Transaction();
  tx.setSenderIfNotSet(input.owner);
