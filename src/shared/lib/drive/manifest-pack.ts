...
import { toWire, type ManifestEntry } from "./manifest-codec.ts";
import { buildIndex, pathOf, type ManifestIndex } from "./manifest-index.ts";
import { CHUNK_PLAIN_MAX } from "./manifest-chunks.ts";
 ...
 */
export function placementKey(entry: ManifestEntry, index: ManifestIndex): string {
...
function reachesRoot(index: ManifestIndex, entry: ManifestEntry): boolean {
 ...
 */
export function comparePlacementKeys(a: string, b: string): number {
  if (a === b) return 0;
...
/** One entry with everything the packer needs, computed once. */
interface Keyed {
  entry: ManifestEntry;
  key: string;
  /** Exact UTF-8 length of this entry's JSON — the same bytes `encodeChunk` will write. */
  bytes: number;
  /** That JSON, for deciding whether a chunk's contents actually changed. */
  json: string;
  astral: boolean;
}

function keyAll(entries: readonly ManifestEntry[]): Keyed[] {
...
function order(a: Keyed, b: Keyed): number {
  const byKey =
    a.astral || b.astral
...
function sizeOf(items: readonly Keyed[]): number {
  let total = CHUNK_DOC_OVERHEAD;
  for (const k of items) total += k.bytes + 1;
 ...
 * conversion (§6.3.6).
 */
export function packAll(entries: readonly ManifestEntry[]): PackedChunk[] {
  return close(fill(keyAll(entries)));
}

/** Greedy fill in key order — the "unused remainder is left unused" rule of §6.3.3. */
function fill(keyed: readonly Keyed[]): Keyed[][] {
  const out: Keyed[][] = [];
  let current: Keyed[] = [];
...
function close(buckets: readonly Keyed[][], reuse: readonly (string | undefined)[] = []): PackedChunk[] {
 ...
 */
export function repack(previous: readonly HeldChunk[], entries: readonly ManifestEntry[]): PackedChunk[] {
  if (previous.length === 0) return packAll(entries);
  const keyed = keyAll(entries);
  const owner = new Map<string, number>();
  for (let i = 0; i < previous.length; i += 1) {
    for (const e of previous[i]?.items ?? []) owner.set(e.id, i);
  }
  const buckets: Keyed[][] = previous.map(() => []);
  for (const k of keyed) {
    const held = owner.get(k.entry.id);
    const home =
      held !== undefined && inRange(k.key, previous[held]) ? held : covering(k.key, previous);
    buckets[home]?.push(k);
  }
  for (const bucket of buckets) bucket.sort(order);
  ...
  if (!ordered(buckets)) return packAll(entries);
...
function inRange(key: string, chunk: HeldChunk | undefined): boolean {
...
function covering(key: string, previous: readonly HeldChunk[]): number {
  let at = 0;
  for (let i = 0; i < previous.length; i += 1) {
    const f = previous[i]?.f;
    if (f !== undefined && comparePlacementKeys(f, key) <= 0) at = i;
  }
  return at;
}

/** Do the buckets, read end to end, still hold every entry in placement order? */
function ordered(buckets: readonly Keyed[][]): boolean {
  let last: Keyed | null = null;
  for (const bucket of buckets) {
    for (const k of bucket) {
      if (last && order(last, k) > 0) return false;
      last = k;
    }
  }
  return true;
...
function unchanged(bucket: readonly Keyed[], chunk: HeldChunk | undefined): boolean {
  if (!chunk || bucket.length !== chunk.items.length || bucket.length === 0) return false;
  for (let i = 0; i < bucket.length; i += 1) {
    const before = chunk.items[i];
    if (!before || before.id !== bucket[i]?.entry.id) return false;
    if (JSON.stringify(toWire(before)) !== bucket[i]?.json) return false;
  }
  return true;
}

/** Split every bucket that outgrew the bound, at the median key, until each one fits. */
function split(buckets: readonly Keyed[][], reuse: readonly (string | undefined)[]): [Keyed[][], (string | undefined)[]] {
  const outBuckets: Keyed[][] = [];
  const outReuse: (string | undefined)[] = [];
  for (let i = 0; i < buckets.length; i += 1) {
 ...
 */
function merge(buckets: readonly Keyed[][], reuse: readonly (string | undefined)[]): [Keyed[][], (string | undefined)[]] {
