 ...
 */
export function applyWalletPatch(next: AccountSettings, activeWallet?: number, walletCount?: number): void {
  if (activeWallet !== undefined && Number.isFinite(activeWallet)) {
    ...
    if (index === ACTIVE_WALLET_DEFAULT) delete next.activeWallet;
    else next.activeWallet = index;
  }
  if (walletCount !== undefined && Number.isFinite(walletCount)) {
    ...
    if (count === WALLET_COUNT_DEFAULT) delete next.walletCount;
    else next.walletCount = count;
  }
  const counted = walletCountOf(next);
  if (counted === WALLET_COUNT_DEFAULT) delete next.walletCount;
  else next.walletCount = counted;
}
