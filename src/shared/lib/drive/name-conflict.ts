...
import { uniqueFileName } from "./unique-name.ts";
...
export function findConflicts(
  ...
  const extra = new Map<string | null, Set<string>>();
  const seen = (parentId: string | null): Set<string> => {
  ...
  batch.forEach((item, at) => {
    ...
    if (already) out.push({ at, name: item.name, parentId: item.parentId });
...
export async function settle(
  ...
  for (const c of found) conflicts.set(c.at, c);

  const handedOut = new Map<string | null, Set<string>>();
  const given = (parentId: string | null): Set<string> => {
  ...
  const takenNow = (parentId: string | null): Set<string> =>
    new Set([...takenIn(parentId), ...given(parentId)]);

  const overwrittenHere = new Map<string | null, Set<string>>();
  const overwritten = (parentId: string | null): Set<string> => {
