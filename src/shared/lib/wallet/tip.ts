...
export function tenthsFromPercent(input: number): number {
...
export function tipFromTenths(paidBaseUnits: bigint, tenths: number): bigint {
  const t = Math.min(TIP_MAX_TENTHS, Math.max(0, Math.round(tenths)));
  if (paidBaseUnits <= 0n || t === 0 || !Number.isFinite(t)) return 0n;
  return (paidBaseUnits * BigInt(t)) / 1000n;
}

/** Does this share need the extra confirmation (a value past the dial's end)? */
export function needsExtraConfirm(tenths: number): boolean {
