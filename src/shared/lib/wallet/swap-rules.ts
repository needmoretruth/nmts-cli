...
import { GAS_BUDGET_MIN_MIST } from "./send-rules.ts";
 ...
 */
export const SWAP_VENUES: readonly SwapVenue[] = ["deepbook", "bluefin"];
...
export const SLIPPAGE_PRESETS_BPS: readonly number[] = [10, 50, 100];
...
export function clampSlippageBps(n: number): number {
  if (!Number.isFinite(n)) return SLIPPAGE_BPS_DEFAULT;
 ...
 */
export function slippageBand(bps: number): SlippageBand {
 ...
 */
export function minOutFromQuote(quoteUnits: bigint, slippageBps: number): bigint {
  if (quoteUnits <= 0n) return 0n;
 ...
 */
export function feeBand(budgetMist: bigint, estimateMist: bigint | null): FeeBand {
  if (estimateMist === null) {
    if (budgetMist > 500_000_000n) return "high";
    return budgetMist < GAS_BUDGET_MIN_MIST ? "low" : "ok";
  }
  if (budgetMist < (estimateMist * 12n) / 10n) return "low";
  if (budgetMist > estimateMist * 10n && budgetMist > 50_000_000n) return "high";
  return "ok";
}
 ...
 */
export function priceDeviationBps(impliedRate: number, marketRate: number): number | null {
  if (!Number.isFinite(impliedRate) || impliedRate <= 0) return null;
...
export function impliedRate(outUnits: bigint, amountInUnits: bigint): number | null {
  if (amountInUnits <= 0n || outUnits <= 0n) return null;
...
export function marketRate(
  ...
  suiUsd: number | null | undefined,
  walUsd: number | null | undefined,
): number | null {
  if (suiUsd == null || walUsd == null || suiUsd <= 0 || walUsd <= 0) return null;
  if (!Number.isFinite(suiUsd) || !Number.isFinite(walUsd)) return null;
 ...
 */
export function maxSwappableSuiMist(suiBalanceMist: bigint, gasBudgetMist: bigint): bigint {
...
export function swapExtremes(input: {
  ...
  const out: SwapExtreme[] = [];
  const slip = slippageBand(input.slippageBps);
  if (slip === "low") out.push("lowSlippage");
  if (slip === "high") out.push("highSlippage");
  if (input.budgetMist !== null) {
    const gas = feeBand(input.budgetMist, input.estimateMist);
    if (gas === "low") out.push("lowFee");
    if (gas === "high") out.push("highFee");
  }
 ...
 */
export function feeRateBpsOf(feeUnits: bigint, amountInUnits: bigint): number | null {
  if (amountInUnits <= 0n || feeUnits < 0n) return null;
 ...
 */
export function deepbookRowFrom(direction: SwapDirection, base: bigint, quote: bigint): DeepbookQuoteRow {
 ...
 */
export function measureDeepbookFeeBps(
  amountInUnits: bigint,
  inputMode: DeepbookQuoteRow,
  deepMode: DeepbookQuoteRow | null,
): number | null {
  if (deepMode === null) return null;
  const spentInput = amountInUnits - inputMode.leftoverInUnits;
  const spentDeep = amountInUnits - deepMode.leftoverInUnits;
  if (spentInput <= 0n || spentDeep <= 0n) return null;
