...
function worthAnotherHost(status: number): boolean {
 ...
 */
export function suiRpcTransport(network: string): JsonRpcHTTPTransport {
  const hosts = suiRpcHosts(network);
  let preferred = 0;
  const fetchWithFailover: typeof fetch = async (input, init) => {
    const asked = new URL(
