...
/** One thing this run is acting on, with the path it was found at. */
interface Target {
  entry: ManifestEntry;
  path: string;
}
...
export async function trashPaths(
  ...
  const found = resolving(() =>
    batchTargets(entries, paths, {
  ...
  for (const t of covered) skipped.push(t.path);
  ...
  try {
    ...
    throw new NmtsError(because, {
      exitCode: 1,
      nextStep:
        `${done} of ${files.length} file rows were moved before this stopped, and the file list was ` +
  ...
  const writing = applyManyToList(input, (now) => {
    ...
    if (verb === "restore" && options.strict === true) refuseTakenNames(now, nowIndex.byId, still);
  ...
  const result = await writing.catch(async (error: unknown) => {
...
function refuseTakenNames(
  ...
  for (const id of ids) {
    ...
    const holder = entries.find(
      (e) => e.id !== id && e.parentId === entry.parentId && normaliseName(e.name) === normaliseName(entry.name) && isLive(index, e),
    );
...
function intentFor(verb: "rm" | "restore", ids: readonly string[], at: number): ManifestIntent {
...
function isUnder(entries: readonly ManifestEntry[], id: string | null, rootId: string): boolean {
...
function uniqueById(files: readonly ManifestEntry[]): ManifestEntry[] {
