 ...
 */
export function placeholderName(id: string): string {
...
function unreadable(): never {
  throw new NmtsError("The server answered with an item listing this version cannot read.", {
    exitCode: 4,
    nextStep:
      "Nothing was written. A row this tool cannot read is a file it would leave out of the " +
...
function optionalString(row: object, name: string): string | null {
  const value: unknown = Reflect.get(row, name);
  if (value === undefined || value === null) return null;
  return typeof value === "string" ? value : unreadable();
}

function itemFrom(row: unknown): SourceItem {
  if (typeof row !== "object" || row === null) return unreadable();
  const size: unknown = Reflect.get(row, "size");
  if (typeof size !== "number" || !Number.isFinite(size)) return unreadable();
 ...
 */
function asItemsAnswer(value: unknown): { items: SourceItem[]; next: string | null } {
  ...
  const rows: readonly unknown[] = raw;
...
async function itemsPage(
  ...
  trash: boolean,
  cursor: string | null,
): Promise<{ items: SourceItem[]; next: string | null }> {
...
async function readListing(
  ...
  for (let page = 0; page < MAX_PAGES; page += 1) {
    ...
    if (answer.next === null) return out;
    if (seenCursors.has(answer.next)) {
      throw new NmtsError("The server's item listing repeated a page marker.", {
        exitCode: 4,
        nextStep:
          "Nothing was written. A listing that does not move cannot be read to the end, and a " +
  ...
  throw new NmtsError("This account has more stored files than one rebuild will read.", {
    exitCode: 4,
    nextStep:
      "Nothing was written. Rebuilding from a listing that stopped early would seal a list " +
 ...
 */
async function serverRowIds(base: string, apiKey: string): Promise<Set<string> | null> {
  ...
  for (let page = 0; page < MAX_PAGES; page += 1) {
    ...
    const raw: unknown = Reflect.get(answer, "objects");
    ...
    const next: unknown = Reflect.get(answer, "next_cursor");
 ...
 */
export async function rebuildFromServer(input: RebuildInput): Promise<RebuiltList> {
  const live = await readListing(input.server, input.apiKey, false, input.onProgress);
  const liveCount = live.length;
  const trashed = await readListing(input.server, input.apiKey, true, (read) =>
    input.onProgress?.(liveCount + read),
  );
