 ...
 */
export const CONFIRM_SCHEMA: Readonly<Record<string, unknown>> = {
  type: "object",
  properties: {
    confirm: {
      type: "boolean",
 ...
 */
export function readAnswer(result: unknown): AskOutcome {
  if (typeof result !== "object" || result === null) return "no";
  const action: unknown = Reflect.get(result, "action");
  if (action !== "accept") return "no";
  const content: unknown = Reflect.get(result, "content");
  if (typeof content !== "object" || content === null) return "no";
...
export function declaredElicitation(capabilities: unknown): boolean {
  if (typeof capabilities !== "object" || capabilities === null) return false;
  const value: unknown = Reflect.get(capabilities, "elicitation");
 ...
 */
export function askerFor(capabilities: unknown, send: SendRequest): Asker {
  if (!declaredElicitation(capabilities)) return null;
  return async (message: string): Promise<AskOutcome> => {
