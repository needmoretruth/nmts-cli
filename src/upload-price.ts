...
import { DEFAULT_PART_BYTES, NCF3_SHAPE, sealedLenFor } from "./seal.ts";
import { paddedPlaintextLen, type PaddingRule } from "./shared/lib/crypto/size-padding.ts";
import { planParts, type PartRange } from "./shared/lib/upload/part-plan.ts";
 ...
 */
export function creditsFor(sealedBytes: number, epochs: number = UPLOAD_EPOCHS): number {
 ...
 */
export function partSizeFor(chosen: string | number | undefined): number {
  if (chosen === undefined) return DEFAULT_PART_BYTES;
...
function parseSize(text: string): number {
  ...
  const scale: Record<string, number> = { b: 1, k: 2 ** 10, kib: 2 ** 10, m: 2 ** 20, mib: 2 ** 20, g: 2 ** 30, gib: 2 ** 30 };
 ...
 */
export function planAndPrice(
  size: number,
  partSize: number,
  rule: PaddingRule,
): { plan: PartRange[]; sealedBytes: number; credits: number; sealFor(range: PartRange): number } {
  const plan = planParts(size, partSize);
  const sealFor = (range: PartRange): number =>
    range.partIndex === plan.length - 1
