...
import { transferTransaction, type TransferShape } from "./wallet-send-chain.ts";
import { storageOpTransaction, type StorageOpShape } from "./storage-control-chain.ts";
import { swapTransaction, type SwapShape } from "./wallet-swap-chain.ts";
...
import type {
  ...
  SignSwap,
  SignTransfer,
} from "./wallet-sign-seams.ts";

// ⛔ RE-EXPORTED UNCHANGED, so every caller keeps one import path: the signing file.
export type {
  SignBlobCertify,
  SignBlobRegister,
  SignMessage,
 ...
 */
async function keypairFor(code: string, index: number): Promise<Ed25519Keypair> {
 ...
 */
export async function signerAddress(code: string, index = BUILT_IN_WALLET_INDEX): Promise<string> {
 ...
 */
export const signExtension: SignExtension = async ({ network, code, wallet, objectIds, epochs }) => {
  ...
  const status: unknown = isRecord(effects) ? effects["status"] : undefined;
  const outcome: unknown = isRecord(status) ? status["status"] : undefined;
  if (outcome === "failure") {
    const why: unknown = isRecord(status) ? status["error"] : undefined;
    throw new NmtsError(
      `The storage network refused the extension: ${typeof why === "string" ? why : "no reason was given"}.`,
      {
        exitCode: 1,
        nextStep:
          `The storage was NOT extended and the file still ends when it did. The transaction fee ` +
 ...
 */
export const signTransfer: SignTransfer = async ({ network, code, wallet, shape }) => {
  const client = walrusClient(network);
  const keypair = await keypairFor(code, wallet);
  ...
  if (outcome === "failure") {
    const why: unknown = isRecord(status) ? status["error"] : undefined;
    throw new NmtsError(
      `The chain refused the transfer: ${typeof why === "string" ? why : "no reason was given"}.`,
      {
        exitCode: 1,
        nextStep:
          `Nothing was sent. The transaction fee was spent. The usual cause is a balance that moved ` +
...
export const signSwap: SignSwap = async ({ network, code, shape }) => {
  ...
  if (refused !== null) {
    throw new NmtsError(`The chain refused the swap: ${refused}.`, {
      exitCode: 1,
      nextStep:
        `Nothing was swapped. The transaction fee was spent. The usual causes are a price that moved ` +
...
function refusedBecause(result: unknown): string | null {
  const effects: unknown = isRecord(result) ? result["effects"] : undefined;
  const status: unknown = isRecord(effects) ? effects["status"] : undefined;
  if (!isRecord(status) || status["status"] !== "failure") return null;
  const why: unknown = status["error"];
 ...
 */
export const signBlobRegister: SignBlobRegister = async ({ network, code, wallet, relayUrl, ...shape }) => {
  ...
  if (refused !== null) {
    throw new NmtsError(`The storage network refused the registration: ${refused}.`, {
      exitCode: 1,
      nextStep:
        `Nothing is stored and no storage was bought. The transaction fee was spent. The usual causes ` +
...
export const signBlobCertify: SignBlobCertify = async ({ network, code, wallet, relayUrl, ...shape }) => {
  const client = payingClient(network, relayUrl);
  const keypair = await keypairFor(code, wallet);
  ...
  if (refused !== null) {
    throw new NmtsError(`The storage network refused the certification: ${refused}.`, {
      exitCode: 1,
      nextStep:
        `The storage is bought and the bytes are on the network; only the certification failed, and ` +
