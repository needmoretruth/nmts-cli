...
export const CONSENTS = {
  ...
  "sign-out": {
    ...
    risk:
      "Whoever is signed in on that device is signed out at once, in the middle of whatever they were " +
  ...
  rollback: {
    ...
    risk:
      "Every change since the previous list was written disappears from the list: uploads since then " +
  ...
  donate: {
    ...
    risk:
      "A gift is voluntary, buys nothing, and is not refunded. It goes to the address the site " +
    ...
    limit:
      "This covers gifts this tool sends: `wallet donate`, and a tip named at a payment. Every gift " +
  ...
  "release-storage": {
    ...
    risk:
      "The bytes stop being served for good. The server allows forty of these a day for the whole " +
 ...
 */
export function readConsentRecords(): Record<string, unknown> {
  try {
...
export function writeConsentRecords(all: Record<string, unknown>): void {
