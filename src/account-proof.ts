 ...
 */
export async function accountProof(code: string): Promise<string> {
 ...
 */
export async function accountProofFor(held: CodeInHand): Promise<string> {
  if (held.source === "env") requireConsent("plain-env");
