...
export function organiseTools(ctx: ToolContext): ToolDefinition[] {
  return [
    {
      name: "nmts_mkdir",
      description:
        "Make a folder in the NMTS account, including any parent folders that are missing. " +
    ...
    },
    {
      name: "nmts_move",
      description:
        "Move one or more entries into a folder in the NMTS account. Nothing is re-uploaded and " +
      ...
      run: (args) =>
        say((write) => mv([...needPaths(args), needString(args, "to")], { ...common(ctx), json: true, write })),
    },
    {
      name: "nmts_rename",
      description:
        "Rename one entry in the NMTS account. The name lives inside the sealed list, so this " +
      ...
      run: (args) =>
        say((write) =>
          rename(needString(args, "path"), needString(args, "name"), { ...common(ctx), json: true, write }),
        ),
    },
    {
      name: "nmts_mark",
      description:
        "Put a mark on files, or take one off. `star` is the account's favourites, `pin` keeps a " +
      ...
      run: (args) => {
        ...
        const on = args["on"];
        if (typeof on !== "boolean") throw new Error("`on` is required: true to mark, false to unmark.");
        const options = { ...common(ctx), json: true } as const;
        return say((write) => {
          const opts = { ...options, write };
          if (mark === "star") return on ? star(paths, opts) : unstar(paths, opts);
          if (mark === "pin") return on ? pin(paths, opts) : unpin(paths, opts);
    ...
    },
    {
      ...
      description:
        "Rename a label on every file that carries it. Changes only the file list; costs nothing.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      run: (args) =>
        say((write) =>
          labelRename(needString(args, "old"), needString(args, "new"), {
            ...common(ctx),
            json: true,
            write,
          }),
    ...
    },
    {
      ...
      description:
        "Take one label off every file that carries it. Changes only the file list; costs nothing.",
      inputSchema: {
        type: "object",
      ...
      run: (args) =>
        say((write) => unlabelAll(needString(args, "name"), { ...common(ctx), json: true, write })),
    },
    {
      name: "nmts_padding",
      description:
        "Read or set how file sizes are hidden on the storage network: standard (a few fixed " +
    ...
    },
    {
      name: "nmts_trash",
      description:
        "Put files in the NMTS account's trash. Nothing is destroyed and nothing is refunded: the " +
