  ...
  return [
    {
      ...
      description:
        "Ask the chain right now about one storage object listed by nmts_losses. Costs nothing. " +
      ...
      run: (args) =>
        say((write) =>
          losses({ ...common(ctx), json: true, recheck: needString(args, "blob_object_id"), write }),
        ),
    },
    {
      name: "nmts_get",
      ...
      inputSchema: {
        ...
        properties: {
          ...
          thumbnail: {
            type: "boolean",
            description:
              "For a video: also send one frame of it (taken by ffmpeg) as a small preview file every " +
