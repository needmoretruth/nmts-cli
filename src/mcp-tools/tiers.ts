...
export function annotationsOf(name: string): Record<string, boolean> {
...
export function tierLine(tier: Tier): string {
  switch (tier) {
    case "none":
      return "";
    case "low":
...
export function cannotAsk(what: string): string {
  return (
    `Refused: ${what} needs the person's yes in this mode, and this client cannot put a question in ` +
 ...
 */
export async function passTool(
  name: string,
  ...
  const t = TOOL_TIERS[name];
  if (t === undefined) return `Refused: ${name} has no tier. Nothing was done.`;
  const act = typeof t.act === "function" ? t.act(args) : t.act;
  const a = ACTS[act];
  if (a.tier === "none" || args["dry_run"] === true) return null;
  if (mode === "skip-permissions") return null;
  if (a.tier === "ultra-high") return `Refused: ${a.what ?? act} is a person's act, in every mode. Nothing was done.`;
  const what = a.what ?? `${name} does this.`;
  if ("lock" in a && !(await unlocked(a.lock, now))) {
    return (
      `Refused: ${what} It is locked on this machine. A person unlocks it at a terminal, once: ` +
  ...
  if (ask === null) return cannotAsk(what);
  const outcome = await ask(t.question?.(args) ?? `${what} Go ahead?`);
  if (outcome === "unreachable") return cannotAsk(what);
...
export function withTiers(tools: readonly ToolDefinition[], asker: () => Asker): ToolDefinition[] {
  return tools.map((tool) => {
    const t = TOOL_TIERS[tool.name];
    const tier: Tier = t === undefined ? "low" : typeof t.act === "function" ? ACTS[t.act({})].tier : ACTS[t.act].tier;
    return {
      ...tool,
