...
import { support } from "../commands/support.ts";
import type { ToolDefinition } from "../mcp.ts";
import { ATTACH_LOG_TEXT, categoryCodes, SUPPORT_SHORT } from "../support-copy.ts";
import { MAX_ATTACHED_RUNS } from "../run-log.ts";
...
const OMIT_ARG = {
  ...
  items: { type: "string" },
  description:
    "Values that must not travel — a file name, a folder path. Every occurrence in the message " +
    "and in the attached log becomes [omitted]. At least 3 characters each.",
} as const;

export function supportTools(ctx: ToolContext): ToolDefinition[] {
  return [
    {
      name: "nmts_support_send",
      description:
        "File a report with the developer of NMTS: a bug, a confusing message, an idea, a " +
      ...
      inputSchema: {
        ...
        properties: {
          ...
          message: {
            type: "string",
            description:
              "The report. A good one has four parts: the command that was run, what was " +
    ...
    },
    {
      ...
      description:
        "The reports this account has filed from the command line, with their codes, what each " +
    ...
    },
    {
      ...
      description:
        "One report and everything said in it, oldest first, using a code from nmts_support_list. " +
      ...
      run: (args) =>
        say((write) => support("show", [needString(args, "code")], { ...common(ctx), json: true, write })),
    },
    {
      name: "nmts_support_reply",
      description:
        `Answer in a report's own thread, using a code from nmts_support_list. ${CARE} The same ` +
      ...
      run: (args) =>
        say((write) =>
          support("reply", [needString(args, "code")], {
            ...common(ctx),
            write,
            yes: true,
            message: needString(args, "message"),
 ...
 */
function runsAsked(value: unknown): { attachLog: string } | Record<string, never> {
