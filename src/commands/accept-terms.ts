...
/** Where this account stands, as `GET /v1/account/summary` tells a tool. */
interface Standing {
  owed: boolean;
  terms: string;
  privacy: string;
...
export async function acceptTerms(options: AcceptTermsOptions = {}): Promise<number> {
  ...
  if (!relayed && options.readLine === undefined && !stdinIsATerminal()) {
    throw new NmtsError("There is no terminal to type into (stdin is not a TTY).", {
      exitCode: 3,
      nextStep:
        `Run this where a person can type the two versions, or — after they have said so — ` +
  ...
  if (standing !== null && !standing.owed) {
    say(
      `This account has already accepted the documents in force ` +
  ...
  say(
    `Read them first: \`${BINARY_NAME} terms\` · \`${BINARY_NAME} privacy\` · what changed: ` +
...
async function standingOf(server: string, apiKey: string): Promise<Standing> {
  const answer = await request(server, "/v1/account/summary", { token: apiKey });
  const terms = isRecord(answer) ? answer["terms"] : undefined;
  if (!isRecord(terms)) throw new NmtsError("The server described the account in a shape this version cannot read.");
