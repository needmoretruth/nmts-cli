...
import { parseWalletGrant, readWalletGrant, walletGrantState, writeWalletGrant } from "../wallet-grant.ts";
...
export async function unlock(
  ...
  if (action === undefined || action === "" || action === "list") {
    ...
    for (const key of CONSENT_KEYS) {
      if (key === "wallet") {
        for (const line of walletLines(now())) say(line);
        continue;
      }
      const at = grantedAt(key);
      say(`${at === null ? "  locked    " : "  unlocked  "}  ${key}`);
      say(`                ${CONSENTS[key].what}`);
      if (at !== null) say(`                unlocked on this machine ${at}`);
      say(``);
    }
  ...
  if (action === "unlock") {
    ...
    if (options.readLine === undefined && !stdinIsATerminal() && currentMode() !== "skip-permissions") {
      throw new NmtsError(`A person unlocks, at a terminal — stdin here is not one.`, {
        exitCode: 5,
        nextStep:
          `If you are an agent: do not unlock things. Show the person what \`${BINARY_NAME} unlock\` ` +
...
function row(key: ConsentKey, now: Date): Record<string, unknown> {
  if (key === "wallet") {
...
function walletLines(now: Date): string[] {
  const g = readWalletGrant();
  const state = walletGrantState(g, now);
  ...
  if (g !== null) {
    ...
    out.push(
      `                ${ceilingWords(g.capWalFrost, g.capSuiMist)} · signed away so far ${coinAmount(BigInt(g.spentWalFrost))} WAL, ${coinAmount(BigInt(g.spentSuiMist))} SUI in fees`,
    );
  } else {
...
function ceilingWords(capWal: string | null, capSui: string | null): string {
  if (capWal === null && capSui === null) return "no ceiling on what this tool may sign away";
  const parts: string[] = [];
  if (capWal !== null) parts.push(`${coinAmount(BigInt(capWal))} WAL`);
  if (capSui !== null) parts.push(`${coinAmount(BigInt(capSui))} SUI`);
