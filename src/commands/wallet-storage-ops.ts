...
export async function walletStorageOps(op: StorageOp, rest: readonly string[], options: StorageOpsOptions = {}): Promise<number> {
  ...
  const outcome = await reshapeStorage(
    { network, code: resolved.code, wallet, address },
    ask,
    {
        ...
        cut: (what, limit) =>
          new NmtsError(
            what === "size"
              ...
              : `--epochs must be a whole number above 0 and below the resource's ${limit} epochs.`,
            { exitCode: 2 },
          ),
      },
      onReview: (review) => {
        if (!options.json) describe(say, review, wallet);
      },
...
function spendOf(review: StorageOpReview): { walFrost: bigint; suiMist: bigint } {
  return { walFrost: 0n, suiMist: review.feeMist ?? 0n };
}

/** The review a person reads, in the order somebody deciding needs it. */
function describe(say: (line: string) => void, review: StorageOpReview, wallet: number): void {
  const { plan } = review;
  say(`Would ${what(plan)}`);
  say(`  from  ${review.address} (wallet ${wallet})`);
  for (const line of detail(plan)) say(`  ${line}`);
  say(
    review.feeMist === null
...
function what(plan: StorageOpPlan): string {
  ...
  if (result.kind === "split") return `cut ${named(0)} by ${plan.shape.kind === "splitSize" ? "size" : "period"}.`;
  if (result.kind === "merge") return `join ${named(1)} into ${named(0)}.`;
  return `hand ${named(0)} to ${result.to}.`;
}

/** What it would leave behind, said from the numbers the plan worked out. */
function detail(plan: StorageOpPlan): string[] {
  const result = plan.result;
  ...
  if (result.kind === "merge") {
    const b: StorageShape = result.becomes;
    return [
      result.how === "amount"
...
function asked(
  ...
  options: StorageOpsOptions,
): { ask: StorageOpAsk; command: string } {
  const [a, b] = rest;
  if (op === "split") {
    if (a === undefined) throw usage("split <resource id> --size <bytes> | --epochs <n>");
    if (options.size !== undefined && options.epochs !== undefined) throw usage("split takes --size OR --epochs, not both");
  ...
  if (op === "merge") {
    if (a === undefined || b === undefined) throw usage("merge <resource id> <resource id>");
    return { ask: { kind: "merge", first: a, second: b }, command: `${BINARY_NAME} wallet storage merge ${a} ${b}` };
  }
  if (a === undefined || b === undefined) throw usage("transfer <resource id> <address>");
  return { ask: { kind: "transfer", objectId: a, to: b }, command: `${BINARY_NAME} wallet storage transfer ${a} ${b}` };
}

function usage(form: string): NmtsError {
...
export function parseBytes(text: string): number {
  ...
  if (m === null || m[1] === undefined) throw new NmtsError("--size is a number of bytes, or one with KiB, MiB or GiB.", { exitCode: 2 });
