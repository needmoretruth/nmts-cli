...
import { canFuse, statusOf } from "../shared/lib/storage-control/plan.ts";
...
export async function walletStorageOps(op: StorageOp, rest: readonly string[], options: StorageOpsOptions = {}): Promise<number> {
  ...
  const held = (id: string): StorageResource => {
...
function plan(
  ...
  held: (id: string) => StorageResource,
): { shape: StorageOpShape; action: WalletAction; lines: { what: string; detail: string[] }; command: string } {
  const [a, b] = rest;
  const resource = (r: StorageResource): string =>
    `${formatBytes(r.sizeBytes)} · epoch ${r.startEpoch} to ${r.endEpoch} (${r.objectId})`;
  if (op === "split") {
