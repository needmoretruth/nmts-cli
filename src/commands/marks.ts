...
/** The six commands, and the three marks they are two halves of. */
type MarkVerb = "star" | "unstar" | "pin" | "unpin" | "label" | "unlabel";

/** Star one or more files: they show in the drive's favourites as well as in their own folder. */
export async function star(paths: readonly string[], options: MarkOptions = {}): Promise<number> {
  return run("star", paths, undefined, options);
}

/** Take the star off. */
export async function unstar(paths: readonly string[], options: MarkOptions = {}): Promise<number> {
  return run("unstar", paths, undefined, options);
}

/** Hold one or more files at the top of the folder they are in. */
export async function pin(paths: readonly string[], options: MarkOptions = {}): Promise<number> {
  return run("pin", paths, undefined, options);
}

/** Let them fall back into the ordinary order. */
export async function unpin(paths: readonly string[], options: MarkOptions = {}): Promise<number> {
 ...
 */
export async function labelRename(
 ...
 */
export async function unlabelAll(
  name: string | undefined,
  options: MarkOptions = {},
): Promise<number> {
  const say = options.write ?? ((line: string) => process.stdout.write(`${line}\n`));
...
/** What one run did, gathered inside the attempt that actually landed. */
interface MarkOutcome {
  /** Paths this run changed. */
  marked: string[];
  /** Paths that already were the way the command asked for, so nothing was written for them. */
  already: string[];
}
...
async function run(
  ...
  const session = await openSession(options);
  const at = Date.now();
  const outcome: MarkOutcome = { marked: [], already: [] };
    ...
    for (const target of targets) {
      if (target.kind !== KIND_FILE) {
        throw new NmtsError(`"${fullPathOf(index, target)}" is a folder, and marks are for files.`, {
          exitCode: 4,
          nextStep:
            `Nothing was marked. A star, a pin and a label are shown on files; a folder that ` +
  ...
  const names = (list: readonly string[]): string => list.map((p) => `"${p}"`).join(", ");
  if (outcome.marked.length > 0) {
    ...
    say(
      `Nothing changed: ${names(outcome.already)} ` +
...
function intentFor(
  ...
  if (verb === "star" || verb === "unstar") return { op: "favorite", ids, on, at };
  if (verb === "pin" || verb === "unpin") return { op: "pin", ids, on, at };
  return { op: "label", ids, label: text, on, at };
}

/** Does this entry already carry the mark the verb is about? */
function wears(entry: ManifestEntry, verb: MarkVerb, text: string): boolean {
  if (verb === "star" || verb === "unstar") return entry.favorite === true;
  if (verb === "pin" || verb === "unpin") return entry.pinned === true;
  return (entry.labels ?? []).includes(text);
}

/** What the run did, as the first word of the sentence saying it. */
function did(verb: MarkVerb, text: string): string {
  if (verb === "star") return "Starred";
  if (verb === "unstar") return "Took the star off";
  if (verb === "pin") return "Pinned";
  if (verb === "unpin") return "Unpinned";
  if (verb === "label") return `Put "${text}" on`;
  return `Took "${text}" off`;
}

/** The state the verb asks for, for the sentence about what was already the case. */
function state(verb: MarkVerb, text: string): string {
  if (verb === "star") return "starred";
  if (verb === "unstar") return "not starred";
  if (verb === "pin") return "pinned";
  if (verb === "unpin") return "not pinned";
  if (verb === "label") return `wearing "${text}"`;
  return `without "${text}"`;
}
