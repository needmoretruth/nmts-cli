 ...
 */
function sourceWords(source: CredentialSource): string {
  switch (source) {
    case "env":
...
export function env(options: EnvOptions = {}): number {
  ...
  let code: ReturnType<typeof resolveAccountCode> = null;
  let key: ReturnType<typeof resolveApiKey> = null;
