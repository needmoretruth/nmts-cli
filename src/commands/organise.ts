...
import { applyManyToList, applyToList, batchTargets } from "../manifest-write.ts";
...
/** What one run of `mv` did, gathered inside the attempt that actually landed. */
interface MoveOutcome {
  /** The things this run moved, in the order they were named. */
  moved: { id: string; name: string }[];
  /** The things that were already in the destination, so nothing was written for them. */
  already: string[];
...
export async function mv(operands: readonly string[], options: OrganiseOptions = {}): Promise<number> {
  ...
  const session = await openSession(options);
  const at = Date.now();
  const outcome: MoveOutcome = { moved: [], already: [], parentId: null };
