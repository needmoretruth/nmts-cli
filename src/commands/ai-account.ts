...
import { accountProof, accountProofFor } from "../account-proof.ts";
...
function childrenOf(answer: unknown): Child[] {
  const list: unknown = isRecord(answer) ? answer["children"] : undefined;
  ...
  for (const row of list) {
    ...
    if (typeof id !== "string" || typeof index !== "number" || typeof created !== "string") continue;
...
/** What every door here sends: which account is asking, proved. */
interface Caller {
  server: string;
  accountId: string;
  authSecret: string;
...
async function caller(options: AiAccountOptions): Promise<Caller> {
...
async function listFor(who: Caller): Promise<Child[]> {
  return childrenOf(
    await request(who.server, "/v1/account/ai-accounts/list-by-code", {
      method: "POST",
 ...
 */
export async function aiAccount(verb: string | undefined, args: ParsedArgs): Promise<number> {
  const options: AiAccountOptions = { server: args.server, json: args.json, yes: args.yes };
  if (verb === undefined || verb === "" || verb === "list") return await list(options);
  if (verb === "create") return await create(args.operands[1], options);
  if (verb === "delete") return await remove(args.operands[1], options);
  throw new NmtsError(`\`${verb}\` is not an ai-account verb.`, {
    exitCode: 2,
    nextStep:
      `The verbs are \`${BINARY_NAME} ai-account list\`, \`${BINARY_NAME} ai-account create [place]\` ` +
      `and \`${BINARY_NAME} ai-account delete <account id>\`. Each needs the NMTS key on this ` +
      `machine; an API key cannot reach any of them.`,
  });
}

export async function list(options: AiAccountOptions = {}): Promise<number> {
...
function placeFor(asked: string | undefined, taken: readonly Child[]): number {
  ...
  for (let place = 1; place <= PLACES; place += 1) if (!used.has(place)) return place;
  throw new NmtsError(`All ${PLACES} places under this account are taken.`, {
    exitCode: 4,
    nextStep:
      `Nothing was made. Erase one with \`${BINARY_NAME} ai-account delete <account id>\` — its place ` +
 ...
 */
export async function create(asked: string | undefined, options: AiAccountOptions = {}): Promise<number> {
  const say = options.write ?? ((line: string) => process.stdout.write(`${line}\n`));
  const who = await caller(options);
  ...
  try {
    ...
    if (error instanceof ServerError && error.code === "AI_ACCOUNTS_NEED_ENABLE") {
      throw new NmtsError(`The first AI account under this account is made in a browser.`, {
        exitCode: 4,
        nextStep:
          `Nothing was made. Turning the feature on costs a human check, which no command-line tool ` +
 ...
 */
export async function remove(id: string | undefined, options: AiAccountOptions = {}): Promise<number> {
  const say = options.write ?? ((line: string) => process.stdout.write(`${line}\n`));
  if (id === undefined || id === "") {
