...
import type { PaddingMode } from "../shared/lib/drive/manifest-ops.ts";
 ...
 */
const WORDS = ["standard", "pow2", "off"] as const;
type PaddingWord = (typeof WORDS)[number];

/** What each one is called in a sentence. */
const CALLED: Readonly<Record<PaddingWord, string>> = {
  standard: "standard",
  pow2: "powers of two",
  off: "off (exact size)",
};
...
const SPELT: Readonly<Record<PaddingWord, PaddingMode>> = {
  standard: "padme",
  pow2: "pow2",
  off: "none",
};

function isWord(value: string): value is PaddingWord {
  return (WORDS as readonly string[]).includes(value);
}

export async function padding(wanted: string | undefined, options: PaddingOptions = {}): Promise<number> {
  ...
  if (wanted === undefined || wanted === "") {
    ...
    const at: PaddingWord = stored === "pow2" ? "pow2" : stored === "none" ? "off" : "standard";
    ...
    say(
      `Anyone can read the size of a piece stored on the storage network; the blank bytes make it ` +
  ...
  const mode: PaddingMode = SPELT[wanted];
  ...
  say(
    `Set to ${CALLED[wanted]}. It applies to what is uploaded next, from every device; files ` +
