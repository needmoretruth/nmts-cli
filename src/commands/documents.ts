...
import { HttpError, request, type TextAnswer } from "../api.ts";
...
/** One row of `GET /api/notices`, in the server's own spelling. */
interface NoticeRow {
  id: string;
  date: string;
  title: string;
...
const LEGAL_IDS = { terms: "terms", privacy: "privacy", board: "board-terms" } as const;
...
export async function runDocuments(
  command: string,
  options: DocumentOptions & { id?: string | undefined },
): Promise<number> {
  if (command === "notices") return await notices(options);
 ...
 */
export async function notices(options: DocumentOptions & { id?: string | undefined } = {}): Promise<number> {
...
async function listNotices(
  ...
  for (const row of rows) say(`${row.date}  ${row.id}  ${row.title}`);
  say("");
  say(
    `Read one with \`${BINARY_NAME} notices <id>\`, or keep it as a dated file with ` +
 ...
 *    refuse.
 */
function noticeRows(answer: unknown): NoticeRow[] {
  const list: unknown = isRecord(answer) ? answer["notices"] : undefined;
  ...
  for (const row of list) {
    ...
    const title: unknown = isRecord(row["title"]) ? row["title"]["en"] : undefined;
...
async function noticeBody(base: string, id: string): Promise<TextAnswer> {
  try {
...
export async function legal(which: "terms" | "privacy", options: DocumentOptions = {}): Promise<number> {
  const say = options.write ?? ((line: string) => process.stdout.write(`${line}\n`));
  const base = resolveServer(options.server);
 ...
 */
function languageOf(lang: string | undefined): "en" | "ko" {
  if (lang === undefined || lang === "en") return "en";
 ...
 */
function keep(answer: TextAnswer, out: string | undefined): string {
  ...
  try {
    ...
    if (error instanceof Error && Reflect.get(error, "code") === "EEXIST") {
      throw new NmtsError(`${destination} is already there.`, {
        exitCode: 4,
        nextStep:
          `Nothing was written, and the copy already there was not touched. Pass --out to write ` +
