...
import { activeWalletOf, walletCountOf } from "../shared/lib/drive/manifest-settings.ts";
import { discoverWallets } from "../shared/lib/wallet/discover.ts";
import {
  coinAmount,
  readBalances,
  walCoinType,
...
export interface WalletListOptions {
  ...
  readWalletSettings?: () => Promise<{ active: number; count: number }>;
}
...
/** One wallet as this command knows it. */
interface Row {
  index: number;
  address: string;
...
export async function walletList(options: WalletListOptions = {}): Promise<number> {
  ...
  const history =
    options.hasHistory ??
    (async (net: Network, addr: string) => (await import("../wallet-list-chain.ts")).hasHistory(net, addr));
  ...
  const scan = await discoverWallets(async (index) => {
    ...
    const held = (b: CoinBalance): boolean => b.read && b.baseUnits > 0n;
    if (held(balances.sui) || held(balances.wal)) return true;
  ...
  for (const row of rows) {
    ...
    say(
      `${String(row.index).padEnd(NUMBER_W)}${row.address.padEnd(ADDRESS_W)}` +
...
async function walletSettings(options: WalletListOptions): Promise<{ active: number; count: number }> {
