 ...
 */
function runInstaller(command: readonly string[], json: boolean): InstallOutcome {
  const [, ...rest] = command;
  ...
  const outcome: InstallOutcome = { code: result.status ?? 1 };
  if (json) {
    const said = `${result.stdout ?? ""}${result.stderr ?? ""}`.trim();
    if (said.length > 0) outcome.output = said.slice(-2000);
  }
  return outcome;
}
...
export async function update(options: UpdateOptions = {}): Promise<number> {
  ...
  if ("failed" in found) throw couldNotAsk(found.failed);
  ...
  if (outcome.unstartable !== undefined) {
    throw new NmtsError(`The installer could not be started: ${outcome.unstartable}`, {
      exitCode: 1,
      nextStep:
        `Nothing was changed. \`npm\` has to be on the PATH for this command to install ` +
  ...
  if (outcome.code !== 0) {
    throw new NmtsError(`The installer stopped with code ${outcome.code}.`, {
      exitCode: 1,
      nextStep:
        (outcome.output !== undefined ? `${outcome.output}\n\n` : "") +
...
function couldNotAsk(why: string): NmtsError {
  return new NmtsError(`Could not find out which release is newest: ${why}.`, {
    exitCode: 1,
    nextStep:
      `Nothing was changed. This asks the site the releases are published on, not NMTS — the ` +
