...
import { folderFor, type PlannedFile, type PushOptions } from "./push.ts";

/** The wallet rail, one file at a time, stopping at the first failure as the credit rail does. */
export async function pushWithWallet(
  ...
  if (!options.json) {
    ...
    if (run.already.length > 0) say(`  ${run.already.length} already there, not sent again`);
  }
  ...
  try {
    for (const one of run.todo) {
      ...
      const done = await uploadOneWithWallet(
        ctx,
        { localPath: one.local, size: one.size, name: one.name, parentId, destination: one.folder },
        { ...options, dryRun: options.dryRun === true },
      );
      if (done !== null) uploaded.push(`${one.folder}/${done.savedAs}`);
    }
  } catch (error) {
    ctx.progress.done();
