...
/** An instant the server named: normalised to UTC for printing, and kept as a number for arithmetic. */
interface Moment {
  iso: string;
  ms: number;
}

/** What the server answers when a code is minted. */
interface Minted {
  code: string;
  verifyUrl: string;
  expiresAt: Moment;
...
/** What the server answers about the account's standing check. */
interface Standing {
  verified: boolean;
...
export async function verify(options: VerifyOptions = {}): Promise<number> {
  ...
  if (quiet) sayResult(say, standing, true);
  throw new NmtsError(`The code stopped working before anybody used it.`, {
    exitCode: 1,
    nextStep:
      `Nothing was verified and nothing was spent. Run \`${BINARY_NAME} verify\` again when the ` +
...
function stopIfCancelled(signal: AbortSignal, expiresAt: Moment): void {
  if (!signal.aborted) return;
  throw new NmtsError(`Cancelled.`, {
    exitCode: 130,
    nextStep:
      `The code is still good until ${expiresAt.iso}, so a person who types it after this still ` +
      `passes the check. \`${BINARY_NAME} verify --status\` says whether they did.`,
  });
}

/** Ask the server about the standing check. */
async function ask(server: string, apiKey: string): Promise<Standing> {
 ...
 */
async function askDuring(server: string, apiKey: string, signal: AbortSignal, expiresAt: Moment): Promise<Standing> {
  try {
...
function sayStanding(say: (line: string) => void, standing: Standing): void {
  if (standing.verified) {
...
function sayVerified(say: (line: string) => void, standing: Standing): void {
...
function sayResult(say: (line: string) => void, standing: Standing, codeExpired: boolean): void {
  say(
    JSON.stringify({
      event: "result",
 ...
 */
function momentOf(raw: string | null): Moment | null {
 ...
 */
function trustedUrl(raw: string | null): string | null {
  if (raw === null) return null;
  let parsed: URL;
  try {
    parsed = new URL(raw);
...
function readMinted(answer: unknown): Minted {
  ...
  if (code === null || verifyUrl === null || expiresAt === null) {
    throw brokenAnswer(
      "a code, an address a browser can trust to type it at, and when it stops working",
    );
  }
...
function readStanding(answer: unknown): Standing {
  const verified = booleanField(answer, "verified");
  if (verified === null) throw brokenAnswer("whether this account is verified");
  return {
    verified,
...
function brokenAnswer(wanted: string): NmtsError {
