...
const COIN_WORDS: Record<SendValidationError, string> = {
...
export async function walletSend(operands: readonly string[], options: WalletSendOptions = {}): Promise<number> {
  ...
  if (!verdict.ok) {
    ...
    throw new NmtsError(COIN_WORDS[verdict.error], {
      exitCode: exit,
      nextStep:
        `Nothing was signed. The wallet holds ${coinAmount(suiBalance)} SUI and ${coinAmount(walBalance)} WAL; ` +
