...
import {
  ...
  type ActivityRow,
  type RpcTransaction,
} from "../shared/lib/wallet/activity.ts";
import { BLUEFIN_PACKAGE_IDS, DEEPBOOK_PACKAGE_IDS } from "../shared/lib/wallet/venue-ids.ts";
...
export function activityContext(network: Network, address: string): ActivityContext {
...
export async function walletActivity(options: WalletActivityOptions = {}): Promise<number> {
  ...
  const query = options.queryChain ?? (async (net: Network, addr: string) => (await import("../wallet-activity-chain.ts")).queryActivity(net, addr));
  let fetched: ActivityFetch;
  try {
    ...
    throw new NmtsError("The wallet's transactions could not be read from the chain.", {
      exitCode: 1,
      nextStep:
        `Nothing is known about this wallet's activity from here — that is not the same as there ` +
  ...
  const rows = mergeActivity(
    [fetched.sent.map((tx) => toActivityRow(tx, ctx)), fetched.received.map((tx) => toActivityRow(tx, ctx))],
    SHOW,
  );
  ...
  if (rows.length === 0) {
    ...
    for (const row of rows) say(`  ${inWords(row)}`);
  }
  say(``);
...
function inWords(row: ActivityRow): string {
...
function asJson(row: ActivityRow): Record<string, unknown> {
  return {
    digest: row.digest,
