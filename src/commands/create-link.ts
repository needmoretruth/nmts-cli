...
export async function createThroughLink(options: CreateLinkOptions): Promise<number> {
  ...
  if (options.wallet !== undefined) {
    const { refuseIfWalletIsTaken } = await import("./openers-attach.ts");
  ...
  const answer: unknown = await request(server, "/v1/accounts/registration-links", {
    method: "POST",
  ...
  if (url === null) throw unexpected("an address");
  ...
  if (!finished) throw nobodyCame(statusUrl);
  say(``);
  ...
  //    down with it.
  if (options.wallet !== undefined) {
    const { attachWalletToNewAccount, sayWalletAttached } = await import("./openers-attach.ts");
...
/** The puzzle this server is setting right now. */
interface Puzzle {
  challenge: string;
  bits: number;
}

async function askForWork(server: string): Promise<Puzzle> {
  const answer: unknown = await request(server, "/v1/accounts/registration-challenge");
 ...
 */
function solve(challenge: string, accountId: string, bits: number, say: (line: string) => void): string {
 ...
 */
async function waitForAPerson(server: string, linkId: string): Promise<boolean> {
  const deadline = Date.now() + WAIT_FOR_MS;
  for (;;) {
    const answer: unknown = await request(
      server,
      `/v1/accounts/registration-links/${linkId}`,
    );
 ...
 */
async function sayTheQr(say: (line: string) => void, url: string): Promise<void> {
 ...
 */
function sayWhatIsNext(say: (line: string) => void): void {
  say(``);
...
function nobodyCame(statusUrl: string): NmtsError {
...
function unexpected(what: string): NmtsError {
...
function stringField(answer: unknown, field: string): string | null {
  const value: unknown = isRecord(answer) ? answer[field] : null;
