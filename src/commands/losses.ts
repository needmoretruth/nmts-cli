...
/** What the server answers a re-check with. Three, because the chain gives three. */
type RecheckResult = "found" | "still_missing" | "unread";
 ...
 */
function noSuchLine(id: string): NmtsError {
...
function rowsOf(answer: unknown): LossRow[] {
  const list: unknown = isRecord(answer) ? answer["losses"] : undefined;
  ...
  return list.map((row: unknown) => {
    const id: unknown = isRecord(row) ? row["blob_object_id"] : undefined;
    const seen: unknown = isRecord(row) ? row["first_seen"] : undefined;
    const notice: unknown = isRecord(row) ? row["required_notice"] : undefined;
    const restricted: unknown = isRecord(row) ? row["restricted"] : undefined;
...
export async function losses(options: LossesOptions = {}): Promise<number> {
  ...
  if (options.recheck !== undefined) {
    ...
    try {
      ...
      if (error instanceof ServerError && error.status === 404) throw noSuchLine(id);
      throw error;
    }
    const result: unknown = isRecord(answer) ? answer["result"] : undefined;
    ...
    const said: RecheckResult = result;
    if (options.json === true) {
      say(JSON.stringify({ blob_object_id: id, result: said }));
      return 0;
    }
    if (said === "found") say(`The chain knows ${id} again. Its line came off.`);
    else if (said === "still_missing") say(`The chain still does not know ${id}. The line stays.`);
    else say(`The chain could not be read just now. Nothing changed.`);
    return 0;
  }
  ...
  if (options.dismiss !== undefined) {
    ...
    say(
      `Took the line for ${id} off. The incident stays in a record that names nobody; if the same ` +
  ...
  for (const row of rows) {
    say(
      `${row.blob_object_id}  first missed ${utcDay(row.first_seen)}` +
  ...
  say(``);
  say(
    `The date is when a check first could not find it, not when the storage went. There is no ` +
      `file name: the server cannot pair the object with a file, and NMTS cannot see inside files.`,
  );
  say(
    `\`${BINARY_NAME} losses --recheck <id>\` asks the chain again now. \`${BINARY_NAME} losses ` +
