...
import { buildIndex, fullPathOf, trashedAt } from "../drive-paths.ts";
import { NmtsError } from "../errors.ts";
import { readFileList } from "../manifest.ts";
import { applyToList } from "../manifest-write.ts";
...
export async function sweep(options: SweepOptions = {}): Promise<number> {
  ...
  if (!held.complete) {
    throw new NmtsError(`This account has more stored files than one sweep will read.`, {
      exitCode: 4,
      nextStep:
        `Nothing was changed. Dropping an entry is only safe once the server has been seen to let ` +
  ...
  const shown = (e: ManifestEntry): string => fullPathOf(index, e);
  const ageDays = (e: ManifestEntry): number => Math.floor((now - (trashedAt(index, e) ?? now)) / DAY_MS);
  ...
  const summary = (dropped: number, changed: boolean, extra: Record<string, unknown> = {}): string =>
    JSON.stringify({
  ...
  if (plan.drop.length === 0) {
    if (options.json) say(summary(0, false));
    else reportHeldBack(say, plan);
    return 0;
  }
  ...
  if (options.yes !== true) {
    ...
    say(
      `${plan.drop.length} thing${plan.drop.length === 1 ? "" : "s"} in the trash passed ` +
    ...
    for (const entry of plan.drop) say(`  ${shown(entry).padEnd(width)}  trashed ${ageDays(entry)} days ago`);
    say(``);
...
function reportHeldBack(say: (line: string) => void, plan: PurgePlan): void {
  if (plan.waiting.length > 0) {
    say(
      `${plan.waiting.length} more ${plan.waiting.length === 1 ? "is" : "are"} past ` +
  ...
  if (plan.tangled.length > 0) {
    say(
      `${plan.tangled.length} folder${plan.tangled.length === 1 ? "" : "s"} above ` +
...
function describe(
  plan: PurgePlan,
  shown: (e: ManifestEntry) => string,
  ageDays: (e: ManifestEntry) => number,
): { id: string; path: string; daysInTrash: number; action: string }[] {
  const rows: { id: string; path: string; daysInTrash: number; action: string }[] = [];
 ...
 */
async function serverItemIds(base: string, apiKey: string): Promise<{ ids: Set<string>; complete: boolean }> {
  ...
  for (let page = 0; page < MAX_PAGES; page += 1) {
    ...
    if (read.next === null) return { ids, complete: true };
    cursor = read.next;
  }
 ...
 */
function asObjectsAnswer(value: unknown): { ids: string[]; next: string | null } {
  ...
  const raw: unknown = Reflect.get(value, "objects");
  if (!Array.isArray(raw)) return unreadable();
  const objects: readonly unknown[] = raw;
  const ids: string[] = [];
  for (const object of objects) {
    if (typeof object !== "object" || object === null) return unreadable();
    const id: unknown = Reflect.get(object, "id");
    if (typeof id !== "string") return unreadable();
    ids.push(id);
  }
  const next: unknown = Reflect.get(value, "next_cursor");
  if (next !== null && next !== undefined && typeof next !== "string") return unreadable();
