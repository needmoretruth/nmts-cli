...
import { createFirstList } from "../manifest-create.ts";
import { accountKeyCheck, type AccountKeyCheck } from "../rebuild-key-check.ts";
import { readFileList } from "../manifest.ts";
import { BINARY_NAME } from "../product.ts";
import { rebuildFromServer, type RebuiltList } from "../rebuild.ts";
import type { PairVerdict, UnverifiedReason } from "../shared/lib/drive/rebuild-verify.ts";
import type { SourceItem } from "../rebuild.ts";
import { openSession } from "../session.ts";
import { TRASH_RETENTION_DAYS } from "../trash-sweep.ts";
...
function summary(built: RebuiltList, wrote: boolean, extra: Record<string, unknown> = {}): string {
...
export async function rebuild(options: RebuildOptions = {}): Promise<number> {
  ...
  if (current.manifest !== null) {
    throw new NmtsError(`This account already has a file list (version ${current.seq ?? 0}).`, {
      exitCode: 4,
      nextStep:
        `Nothing was changed. Rebuilding is for an account whose list is missing; over an existing ` +
  ...
  if (!current.firstTimeOnThisMachine && options.force !== true) {
    throw new NmtsError(`This machine has seen a file list for this account, and the server now says there is none.`, {
      exitCode: 4,
      nextStep:
        `Nothing was changed. A list that was there and is not is different from one that never ` +
  ...
  let checker: AccountKeyCheck | null = null;
  ...
  try {
    ...
    built = await rebuildFromServer({
      ...
      verify,
      onProgress: (read) => {
        if (options.json === true || read < announced + 1000) return;
        announced = read;
        say(`  read ${read} stored files so far...`);
      },
      onVerifyProgress: (checked, total) => {
        if (options.json === true || checked < checkedAnnounced + 1000) return;
  ...
  say(
    `Wrote file list version ${written.seq}: ${built.entries.length} ` +
...
function describe(say: (line: string) => void, built: RebuiltList): void {
  say(
    `This account has no file list, and the server still holds ${built.entries.length} ` +
 ...
 */
function reportKeys(say: (line: string) => void, built: RebuiltList): void {
...
function describeReasons(built: RebuiltList): string {
  const counts = new Map<UnverifiedReason, number>();
  for (const one of built.unverified) counts.set(one.reason, (counts.get(one.reason) ?? 0) + 1);
...
function reportGaps(say: (line: string) => void, built: RebuiltList): void {
  if (built.keyless > 0) {
    say(``);
    say(
      `  ⚠ ${built.keyless} of them carr${built.keyless === 1 ? "ies" : "y"} no key on the server.` +
  ...
  if (built.unaccounted > 0) {
    say(``);
    say(
      `  ⚠ ${built.unaccounted} row${built.unaccounted === 1 ? "" : "s"} the server holds ` +
