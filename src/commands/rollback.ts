...
/** What `GET /v1/manifest` and `GET /v1/manifest/previous` both answer. Narrowed, not trusted. */
type Version = { state: "absent" } | { state: "present"; seq: number; ct: string };

function asVersion(value: unknown): Version {
  if (!isRecord(value)) {
...
function nothingRetained(): NmtsError {
  return new NmtsError("The server holds no previous version of the file list.", {
    exitCode: 4,
    nextStep:
      `Nothing was changed. \`${BINARY_NAME} rebuild\` builds a list from the server's rows when ` +
...
export async function rollback(options: RollbackOptions = {}): Promise<number> {
  ...
  const previous = asVersion(
    await request(session.server, "/v1/manifest/previous", { token: session.apiKey }),
  );
  if (previous.state === "absent") throw nothingRetained();
  ...
  if (current.state === "absent") throw nothingRetained();
  ...
  if (options.yes !== true) {
    ...
    say(
      `Version ${previous.seq} of the file list would go back as the current one, over version ` +
        `${current.seq}.`,
    );
    say(``);
  ...
  say(
    `Put version ${previous.seq} of the file list back as the current one, over version ` +
