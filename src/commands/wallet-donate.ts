...
export const GIFT_NOTICES: readonly string[] = [
...
export function asDonationConfig(value: unknown): DonationConfig {
  if (!isRecord(value)) throw new NmtsError("The server's donation answer is not the shape this version reads.", { exitCode: 1 });
...
function coinOf(raw: string | undefined): SendCoin {
...
export async function walletDonate(operands: readonly string[], options: WalletDonateOptions = {}): Promise<number> {
  ...
  const config = await (options.readDonation ?? (async (base: string) => asDonationConfig(await request(base, "/api/donation"))))(server);
  ...
  const reads = await (options.readChain ?? (async (net: Network) => (await import("../wallet-send-chain.ts")).sendReads(net)))(network);
  ...
  const gasBudgetMist =
    options.feeCap === undefined ? undefined : clampGasBudgetMist(parseTokenAmountToBaseUnits(options.feeCap) ?? 0n);
  const shape: TransferShape = {
    coin,
  ...
  if (!options.json) {
    ...
    say(``);
    for (const line of GIFT_NOTICES) say(`  ${line}`);
  }
