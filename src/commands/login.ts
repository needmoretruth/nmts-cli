...
export async function login(options: LoginOptions = {}): Promise<number> {
  ...
  const exit = await holdTerminal(async () => {
    ...
    if (options.env === true) return printEnvForm(code, say);
 ...
 */
function printEnvForm(code: string, say: (line: string) => void): number {
  requireConsent("plain-env");
 ...
 */
function sayAboutTheKey(outcome: KeyOutcome, server: string, say: (line: string) => void): void {
  switch (outcome.kind) {
    case "unchanged":
