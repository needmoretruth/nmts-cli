...
export function stageOf(clock: EpochClock, expiryEpoch: number, nowMs: number): ExpiryStage {
  ...
  if (!Number.isFinite(expiryEpoch) || expiryEpoch <= 0) return "unrecorded";
  if (expiryEpoch <= clock.current) return "lapsed";
  if (expiryEpoch <= clock.current + warningEpochs(clock, URGENT_DAYS, URGENT_EPOCHS)) return "urgent";
  if (expiryEpoch < clock.current + warningEpochs(clock, NOTICE_DAYS, NOTICE_EPOCHS)) return "soon";
  return "later";
}
 ...
 */
export function warningCutoffEpoch(clock: EpochClock): number {
 ...
 */
export function daysLeftInWords(left: DaysLeft): string {
  if (left.days < 0) return left.exact ? "already ended" : "may already have ended";
  if (left.days < 1) return left.exact ? "ends today" : "could end today";
