 ...
 */
export async function humanCheck(server: string, apiKey: string): Promise<HumanCheck> {
  const answer: unknown = await request(server, "/v1/agent/verify", { token: apiKey });
 ...
 */
export function askAPersonToVerify(whatIsRefused: string): NmtsError {
  return new NmtsError(`${whatIsRefused} until somebody passes this account's human check.`, {
    exitCode: 4,
    nextStep:
      `Run \`${BINARY_NAME} verify\`. It prints a short code and an address; a person opens that ` +
