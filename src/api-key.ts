...
import { API_KEY_ENV_VAR, API_KEY_FILE_ENV_VAR, readSecretFile } from "./credentials.ts";
...
export type KeyShape =
  | { readonly kind: "key"; readonly handle: string }
 ...
 */
export function wellFormed(value: string): KeyShape {
  if (!value.startsWith(KEY_PREFIX)) return { kind: "not-a-key" };
  if (value.length !== KEY_LEN) return { kind: "malformed" };
  if (value[SEPARATOR_AT] !== "_") return { kind: "malformed" };
  const handle = value.slice(KEY_PREFIX.length, SEPARATOR_AT);
  const secret = value.slice(SEPARATOR_AT + 1);
  if (!B64URL.test(handle) || !B64URL.test(secret)) return { kind: "malformed" };
...
export type KeyOutcome =
  | { readonly kind: "none" }
  | { readonly kind: "unchanged"; readonly apiKey: string }
  | {
  ...
  | { readonly kind: "kept"; readonly apiKey: string; readonly from: KeyOffer };
...
export function keyToStore(outcome: KeyOutcome): string | undefined {
...
export function keySourceName(from: KeyOffer): string {
  switch (from) {
    case "secret-file":
 ...
 */
export async function settleApiKey(intake: KeyIntake): Promise<KeyOutcome> {
  ...
  if (stored !== undefined) {
    ...
    if (offered.value === stored) return { kind: "unchanged", apiKey: stored };
    if (!(await agreedToReplace(intake))) return { kind: "kept", apiKey: stored, from: offered.from };
  }
  const shape = wellFormed(offered.value);
  if (shape.kind !== "key") throw await refuse(shape, offered.value);
 ...
 */
async function offer(intake: KeyIntake): Promise<{ value: string; from: KeyOffer } | null> {
  const fromFile = readSecretFile(API_KEY_FILE_ENV_VAR);
  if (fromFile !== null) return { value: fromFile, from: "secret-file" };
  const fromEnv = process.env[API_KEY_ENV_VAR]?.trim();
  if (fromEnv !== undefined && fromEnv.length > 0) return { value: fromEnv, from: "env" };

  if (intake.stored !== undefined) return null;
 ...
 */
async function agreedToReplace(intake: KeyIntake): Promise<boolean> {
  const ask =
    intake.confirmReplace ??
...
function terminalAsk(question: string): (() => Promise<string>) | null {
  if (!stdinIsATerminal()) return null;
 ...
 */
const KEY_IS_THE_PROBLEM: ReadonlySet<string> = new Set([
  "UNAUTHORIZED",
  "API_KEY_MALFORMED",
  "API_KEY_REVOKED",
  "API_KEY_EXPIRED",
 ...
 */
async function checkedByTheServer(server: string, key: string): Promise<boolean> {
  let answer: unknown;
  try {
  ...
  if (typeof answer === "object" && answer !== null) {
    const verified: unknown = Reflect.get(answer, "verified");
  ...
  throw new NmtsError(`${server} answered, but not the way an NMTS server does.`, {
    exitCode: 1,
    nextStep:
      `The answer did not say whether this account's human check is live, which that address ` +
...
function notWritten(error: unknown): unknown {
...
function joinSteps(...parts: readonly (string | null)[]): string {
 ...
 */
async function refuse(shape: KeyShape, value: string): Promise<NmtsError> {
  if (shape.kind === "malformed") {
    return new NmtsError(`That is not a whole NMTS API key.`, {
      exitCode: 2,
      nextStep:
        `A key is ONE line of ${KEY_LEN} characters beginning \`${KEY_PREFIX}\`; ${value.length} ` +
  ...
  return new NmtsError(`That is not an NMTS API key.`, {
    exitCode: 2,
    nextStep:
      `A key is one line of ${KEY_LEN} characters beginning \`${KEY_PREFIX}\`, made on the ` +
 ...
 */
async function looksLikeAnAccountCode(value: string): Promise<boolean> {
  try {
