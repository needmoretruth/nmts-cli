...
import { readBalances, walCoinType } from "./wallet.ts";
 ...
 */
export const RELAY_TIP_CEILING_MIST: Readonly<Record<Network, number>> = {
  testnet: 100_000,
  mainnet: 250_000_000,
};

/** The Walrus package addresses of one network. Exported from the SDK; the system object survives upgrades. */
export function packageConfig(network: Network): { systemObjectId: string } {
...
export function payingClient(network: Network, relayUrl: string): PayingClient {
...
export async function registerTransaction(
  client: PayingClient,
  network: Network,
  input: RegisterShape & { sender: string },
): Promise<Transaction> {
  const tx = new Transaction();
  tx.setSender(input.sender);
  const { part } = input;
  tx.add(
    client.walrus.sendUploadRelayTip({ size: part.sealedLen, blobDigest: part.blobDigest, nonce: part.nonce }),
  );
  if (input.storage.kind === "buy") {
    const blob = tx.add(
      client.walrus.registerBlob({
        size: part.sealedLen,
        epochs: input.epochs,
        blobId: part.blobId,
...
export function certifyTransaction(
  client: PayingClient,
  input: { blobId: string; blobObjectId: string; certificate: Certificate },
): Transaction {
 ...
 */
export async function createdBlob(
  client: PayingClient,
  objectChanges: unknown,
): Promise<{ blobObjectId: string; endEpoch: number }> {
  const created = Array.isArray(objectChanges)
    ...
    : undefined;
  const id: unknown = created === undefined ? undefined : Reflect.get(created, "objectId");
  if (typeof id !== "string") {
...
export async function encodedLength(client: PayingClient, sender: string, sealedLen: number): Promise<number> {
...
function placeholderPart(sealedLen: number): RegisterShape["part"] {
  return {
    sealedLen,
...
export function walletUploadReads(network: Network, relayUrl: string): WalletUploadReads {
  const client = build(network, relayUrl);
  return {
    readWindow: () => extendReads(network).readWindow(),
    async quoteParts(sealedLens: readonly number[], epochs: number): Promise<PartQuote[]> {
      return Promise.all(
        sealedLens.map(async (sealedLen): Promise<PartQuote> => {
    ...
    readWallet: (address: string) => readBalances(chainReader(network, address), walCoinType(network)),
    async estimateRegisterGas({ sender, sealedLen, epochs, storage }): Promise<bigint | null> {
    ...
    async readStorage(address: string): Promise<StorageResource[]> {
      const type = await readStorageType(client, packageConfig(network).systemObjectId);
      return readOwnedStorage(client, address, type);
    },
    encodedLength: (sender: string, sealedLen: number) => encodedLength(client, sender, sealedLen),
  };
}
