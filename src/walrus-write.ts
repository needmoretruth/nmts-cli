 ...
 */
export function relayTimeoutMs(bodyBytes: number): number {
  const perMiB = 20_000;
  const floor = 60_000;
...
type Extended = ReturnType<typeof extend>;
 ...
 */
export function createBlobProtocol(
  network: string,
  bodyBytes: number,
  onSent?: (sent: number, total: number) => void,
): BlobProtocol & { relayUrl: string } {
  const relayUrl = relayHost(network);
  const client: Extended = extend(network, relayUrl, bodyBytes, onSent);
  return {
    relayUrl,
    async computeMetadata({ bytes, nonce }): Promise<BlobMeta> {
      ...
      const meta = await client.walrus
        ...
        .catch((error: unknown) => fail("The storage network's encoder could not prepare this file", error));
      return {
        blobId: meta.blobId,
    ...
    async uploadToRelay({ blobId, bytes, nonce, registerTxDigest, blobObjectId }): Promise<Certificate> {
 ...
 */
export async function readCurrentEpoch(network: string): Promise<number | null> {
  try {
    const base = new SuiJsonRpcClient({
      network: network === "mainnet" ? "mainnet" : "testnet",
      url: suiRpcHost(network),
    ...
    const epoch: unknown = state.committee.epoch;
 ...
 */
export async function readEpochWindow(network: string): Promise<EpochClock | null> {
  try {
    const base = new SuiJsonRpcClient({
      network: network === "mainnet" ? "mainnet" : "testnet",
      url: suiRpcHost(network),
