...
import { objectsOf } from "./listing.ts";
...
export function isKeyConflict(error: unknown): boolean {
...
export async function hashOfFile(path: string): Promise<Uint8Array> {
...
export async function recordedHash(
  ...
  if (contentHashCt === undefined || contentHashCt === "") return null;
  ...
  try {
    return crypt.envelope_open(
      ...
      Buffer.from(contentHashCt, "base64url"),
    );
  } catch {
 ...
 */
export async function verdictForKey(
  ...
  if (standing === undefined) return "free";
...
export function compare(recorded: Uint8Array | null | undefined, arriving: Uint8Array): SameFile {
  if (recorded === undefined) return "free";
  if (recorded === null) return "unknown";
  if (recorded.length !== arriving.length) return "differs";
  for (let i = 0; i < recorded.length; i += 1) {
    if (recorded[i] !== arriving[i]) return "differs";
  }
  return "same";
}
 ...
 */
export function refusalFor(verdict: "differs" | "unknown", key: string): KeyConflict {
  return new KeyConflict(
    verdict === "differs"
