...
import { buildIndex, fullPathOf, isLive, KIND_FOLDER } from "../drive-paths.ts";
 ...
 *    reported as corrupt.
 */
export function etagOf(entry: ManifestEntry): string {
...
export function objectsOf(entries: readonly ManifestEntry[]): DriveObject[] {
  const index = buildIndex(entries);
  const rows: DriveObject[] = [];
  for (const entry of entries) {
    if (entry.kind === KIND_FOLDER) continue;
    if (!isLive(index, entry)) continue;
    rows.push({
...
export function folderPrefixesOf(entries: readonly ManifestEntry[]): string[] {
  const index = buildIndex(entries);
  const out: string[] = [];
  for (const entry of entries) {
    if (entry.kind !== KIND_FOLDER) continue;
...
export function listObjects(
  ...
  const rows: Array<{ sort: string; row: ObjectRow | null; prefix: string | null }> = [];

  for (const object of objects) {
    if (!object.key.startsWith(query.prefix)) continue;
  ...
  if (query.delimiter.length > 0) {
    for (const folder of folders) {
      if (!folder.startsWith(query.prefix)) continue;
  ...
  for (const item of page) {
    if (item.row !== null) contents.push(item.row);
    else if (item.prefix !== null) commonPrefixes.push(item.prefix);
  }
