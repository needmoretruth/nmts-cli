...
import { AAD, DERIVED, loadCrypto, type CryptoGlue } from "./crypto.ts";
import { NmtsError } from "./errors.ts";
import { pruneChunkCache, readCachedChunk, writeCachedChunk } from "./manifest-chunk-cache.ts";
import { keepTrying } from "./net-retry.ts";
import { registerNodeZstd } from "./zstd-node.ts";
import {
  AAD_FILE_LIST_CHUNK,
  chunkFingerprint,
  decodeChunk,
  ...
  type ManifestIndexV2,
} from "./shared/lib/drive/manifest-chunks.ts";
import type { ManifestEntry } from "./shared/lib/drive/manifest-codec.ts";
import { packAll, repack, type HeldChunk } from "./shared/lib/drive/manifest-pack.ts";
...
export class ManifestChunkError extends NmtsError {
  constructor(message: string) {
    super(message, {
      exitCode: 1,
      nextStep:
        "Nothing was changed. The list is stored in pieces and one of them did not arrive whole, " +
...
function sealUnder(io: ChunkIO, aad: string, body: Uint8Array): string {
  return Buffer.from(io.crypt.envelope_seal(io.key, utf8.encode(aad), body)).toString("base64url");
}

function openUnder(io: ChunkIO, aad: string, ct: string): Uint8Array {
...
function ctOf(answer: unknown): string {
  const ct: unknown = typeof answer === "object" && answer !== null ? Reflect.get(answer, "ct") : null;
 ...
 */
export async function openChunks(io: ChunkIO, index: ManifestIndexV2): Promise<HeldChunk[]> {
  registerNodeZstd();
  const refs = index.chunks;
  const out: HeldChunk[] = new Array<HeldChunk>(refs.length);
  let next = 0;
...
async function openOne(
  ...
  if (ct === null) {
    try {
      ct = ctOf(
        await request(io.server, `/v1/manifest/chunks/${ref.h}`, { token: io.apiKey }),
      );
    } catch (error) {
      ...
      throw new ManifestChunkError(
        `Part of the file list could not be fetched: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  if (doc.seq > indexSeq) {
    throw new ManifestChunkError(
      `Part of the file list claims version ${doc.seq}, after the index's ${indexSeq}.`,
    );
  }
  if (doc.items.length !== ref.n) {
    throw new ManifestChunkError(
      `Part of the file list holds ${doc.items.length} entries where the index says ${ref.n}.`,
    );
  }
  if (cached === null) writeCachedChunk(io.accountId, ref.h, ct);
  return doc.items;
}
 ...
 */
export async function writeChunkedList(io: ChunkIO, plan: ListWritePlan): Promise<ListWriteResult> {
  registerNodeZstd();
...
async function putIndex(
  ...
  for (let attempt = 0; ; attempt += 1) {
    try {
      return seqOf(
        await request(io.server, "/v1/manifest", {
          method: "PUT",
          token: io.apiKey,
 ...
 */
async function putChunks(io: ChunkIO, chunks: ReadonlyMap<string, string>): Promise<void> {
  ...
  const worker = async (): Promise<void> => {
    for (;;) {
      ...
      await keepTrying(
        () =>
          request(io.server, `/v1/manifest/chunks/${name}`, {
            method: "PUT",
            token: io.apiKey,
            body: { ct },
          }),
 ...
 */
export async function namedChunks(code: string, ct: string): Promise<string[]> {
  registerNodeZstd();
 ...
 */
export async function keptChunks(
  code: string,
  accountId: string,
  indexCt: string,
): Promise<string[] | null> {
  ...
  for (const name of names) {
    ...
    if (ct === null) {
      throw new NmtsError("This machine holds the file list's index but not all of its parts.", {
        exitCode: 4,
        nextStep:
          "Nothing was written. Run `nmts ls` once while this machine can reach the server: a " +
